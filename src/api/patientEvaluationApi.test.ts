...
import { mapSubmissionRaw } from "./patientEvaluationApi";
import type { IEvaluationSubmissionRaw } from "@/common/types/patientEvaluationSubmission";

describe("mapSubmissionRaw", () => {
  const baseRaw: IEvaluationSubmissionRaw = {
    id: "sub-1",
    evaluation: "meas-1",
    evaluation_name: "Blood Pressure",
  ...
  it("maps all fields correctly", () => {
    ...
    expect(result.evaluationId).toBe("meas-1");
    expect(result.evaluationName).toBe("Blood Pressure");
  ...
  it("defaults evaluation_name to empty string", () => {
    const raw = { ...baseRaw, evaluation_name: undefined };
    expect(mapSubmissionRaw(raw).evaluationName).toBe("");
  });
