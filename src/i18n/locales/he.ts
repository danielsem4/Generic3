const he = {
  // Navigation & Sidebar
  nav: {
    dashboard: "לוח בקרה",
    patients: "מטופלים",
    clinics: "מרפאות",
    modules: "מודולים",
    medications: "תרופות",
    settings: "הגדרות",
    reports: "דוחות",
    doctors: "רופאים",
    clinicManagers: "מנהלי מרפאות",
    manage: "ניהול",
    analytics: "אנליטיקה",
    clinicModules: "מודולים של המרפאה",
    pageNotReady: "דף {{title}} עדיין לא קיים!",
    logout: "התנתקות",
    logoutConfirmTitle: "האם אתה בטוח שברצונך להתנתק?",
    logoutConfirmDesc: "תנותק מהמערכת ותועבר למסך ההתחברות.",
    cancel: "ביטול",
    logoutConfirm: "כן, התנתק",
    hello: "שלום, {{name}}",
    clinic: "מרפאה",
  },

  // Login
  login: {
    title: "התחברות לחשבון שלך",
    description: "הזן את האימייל שלך למטה כדי להתחבר לחשבונך",
    email: "אימייל",
    password: "סיסמה",
    placeholder: "m@example.com",
    forgotPassword: "שכחת את הסיסמה?",
    rememberMe: "זכור אותי",
    welcomeBack: "ברוך שובך",
    submit: "התחברות",
    submitting: "מתחבר...",
    errorMissing: "אנא מלא את כל השדות.",
    errorFailed: "ההתחברות נכשלה. אנא בדוק את פרטי ההתחברות ונסה שוב.",
    successTitle: "ההתחברות הצליחה!",
    successDesc: "מעביר לדף הבית...",
    failedTitle: "ההתחברות נכשלה",
    failedDesc: "אנא בדוק את פרטי ההתחברות.",
    selectClinic: "בחר מרפאה",
    selectClinicDesc: "בחר את המרפאה שברצונך לנהל בהפעלה זו.",
    twoFaTitle: "אימות דו-שלבי",
    twoFaDesc: "הכנס את הקוד בן 6 הספרות שנשלח לדוא\"ל שלך.",
    verifying: "מאמת...",
    confirm: "אשר",
  },

  // Forgot Password
  forgotPassword: {
    title: "איפוס הסיסמה",
    description: "הזן את הדוא\"ל שלך ונשלח לך קוד אימות.",
    emailLabel: "דוא\"ל",
    emailPlaceholder: "m@example.com",
    sendCode: "שלח קוד",
    sending: "שולח...",
    newPasswordTitle: "הגדרת סיסמה חדשה",
    newPasswordDesc: "בחר סיסמה חזקה לחשבונך.",
    newPasswordLabel: "סיסמה חדשה",
    newPasswordPlaceholder: "********",
    confirmPasswordLabel: "אימות סיסמה",
    confirmPasswordPlaceholder: "********",
    submit: "אפס סיסמה",
    submitting: "מאפס...",
    backToLogin: "חזרה להתחברות",
    successTitle: "הסיסמה אופסה!",
    successDesc: "כעת תוכל להתחבר עם הסיסמה החדשה.",
    errorEmailFailed: "לא הצלחנו לשלוח קוד לדוא\"ל זה. נסה שוב.",
    errorResetFailed: "איפוס הסיסמה נכשל. נסה שוב.",
    errorCodeInvalid: "קוד שגוי או שפג תוקפו. נסה שוב.",
    errorCodeNotVerified: "יש לאמת את הקוד לפני הגדרת סיסמה חדשה.",
    codeTitle: "הזן קוד אימות",
    codeDesc: "הזן את הקוד בן 6 הספרות ששלחנו לדוא\"ל שלך.",
    resend: "שלח קוד מחדש",
    resendSuccess: "קוד חדש נשלח לדוא\"ל שלך.",
  },

  // Home / Dashboard
  home: {
    loading: "טוען נתונים...",
    error: "שגיאה בטעינת נתונים",
    patients: "מטופלים",
    clinics: "מרפאות",
    modules: "מודולים",
    medications: "תרופות",
    clinicManagers: "מנהלי מרפאות",
    doctors: "רופאים",
    evaluations: "הערכות",
    latestUsers: "משתמשים אחרונים",
    updatedNow: "עודכן כעת.",
    firstName: "שם פרטי",
    lastName: "שם משפחה",
    email: "אימייל",
    phone: "מספר טלפון",
    role: "תפקיד",
    view: "צפייה",
    viewPatient: "צפה במטופל",
    edit: "עריכה",
    delete: "מחיקה",
    noData: "אין נתוני פרופיל משתמש זמינים.",
    showing: "מציג {{count}} מתוך {{total}} רשומות.",
  },

  // Patients
  patients: {
    title: "ניהול מטופלים",
    description: "צפייה וניהול כל רשומות המטופלים במערכת",
    addNew: "הוספת מטופל חדש",
    searchPlaceholder: "חיפוש לפי שם, אימייל, ת.ז או טלפון...",
    tableTitle: "רשימת מטופלים רשומים",
    noResults: "לא נמצאו מטופלים התואמים את החיפוש שלך.",
    columnName: "שם מלא",
    columnEmail: "כתובת אימייל",
    columnPhone: "מספר טלפון",
    columnStatus: "סטטוס / תפקיד",
    columnActions: "פעולות",
    loading: "טוען רשומות מטופלים...",
    columnId: "תעודת זהות",
    columnLastVisit: "ביקור אחרון",
    statusActive: "פעיל",
    statusPending: "ממתין לבדיקה",
    statusFollowUp: "במעקב",
    view: "צפייה בפרטים",

    errFirstName: "שם פרטי הוא שדה חובה",
    errLastName: "שם משפחה הוא שדה חובה",
    errEmail: "כתובת אימייל לא תקינה",
    errPhone: "חובה להזין 10 ספרות",
    errPassMin: "הסיסמה חייבת להכיל 6 תווים",
    errPassMatch: "הסיסמאות אינן זהות",
    errPassUpper: "חייב להכיל לפחות אות גדולה אחת",
    errPassNumber: "חייב להכיל לפחות ספרה אחת",
    errPassSpecial: "חייב להכיל לפחות תו מיוחד אחד (!@#$%^&*)",

    togglePatient: "מטופל",
    toggleResearch: "מטופל מחקר",

    registerTitle: "הרשמה",
    registerButton: "הרשם",

    firstNameLabel: "שם פרטי",
    lastNameLabel: "שם משפחה",
    emailLabel: "אימייל",
    phoneNumberLabel: "טלפון",
    passwordLabel: "סיסמה",
    confirmPasswordLabel: "אימות סיסמה",

    firstNamePlaceholder: "",
    lastNamePlaceholder: "",
    emailPlaceholder: "",
    phoneNumberPlaceholder: "",
    passwordPlaceholder: "********",
    confirmPasswordPlaceholder: "********",
    save: "שמור שינויים",
    patientAlreadyExistsError: "המטופל כבר קיים או שהפרטים אינם תקינים",
    patientAddedSuccess: "המטופל נוסף בהצלחה",

    editTitle: "עריכת מטופל",
    deleteTitle: "מחיקת מטופל",
    deleteDescription: "האם אתה בטוח שברצונך למחוק את {{name}}? לא ניתן לבטל פעולה זו.",
    saveChanges: "שמור שינויים",
    cancel: "ביטול",
    saveError: "שמירת השינויים נכשלה. אנא נסה שוב.",
    deleteError: "מחיקת המטופל נכשלה. אנא נסה שוב.",
  },

  // Clinic Managers
  clinicManagers: {
    description: "צפייה וניהול כל מנהלי המרפאות במערכת",
    searchPlaceholder: "חיפוש לפי שם, אימייל או טלפון...",
    addNew: "הוסף מרפאה חדשה",
    registerTitle: "יצירת מרפאה חדשה",
    registerButton: "צור",
    back: "חזרה",
    next: "הבא",
    creating: "יוצר...",
    clinicNameLabel: "שם המרפאה",
    clinicNamePlaceholder: "",
    clinicUrlLabel: "כתובת אתר",
    clinicUrlPlaceholder: "https://www.hamarpea.co.il",
    clinicImageUrlLabel: "קישור לתמונת המרפאה",
    clinicImageUrlPlaceholder: "https://example.com/logo.png",
    clinicTypeLabel: "סוג מרפאה",
    typeSTANDARD: "רגיל",
    typeRESEARCH: "מחקר",
    modulesSearchPlaceholder: "חיפוש מודולים...",
    modulesSelected: "{{count}} מודולים נבחרו",
    modulesLoadError: "טעינת המודולים נכשלה. אנא נסה שוב.",
    managerFirstNameLabel: "שם פרטי",
    managerLastNameLabel: "שם משפחה",
    managerEmailLabel: "אימייל",
    managerPhoneLabel: "מספר טלפון",
    managerPasswordLabel: "סיסמה",
    managerFirstNamePlaceholder: "",
    managerLastNamePlaceholder: "",
    managerEmailPlaceholder: "",
    managerPhonePlaceholder: "",
    managerPasswordPlaceholder: "",
    errPassword: "הסיסמה חייבת להכיל לפחות 8 תווים",
    errImageUrl: "נא להזין כתובת URL תקינה",
    step1: "פרטי המרפאה",
    step2: "מודולי המרפאה",
    step3: "מנהל המרפאה",
    successTitle: "המרפאה נוצרה!",
    successDesc: "המרפאה והמנהל שלה נוצרו בהצלחה.",
    successClose: "סגור",
    errClinicName: "שם המרפאה חייב להכיל לפחות 2 תווים",
    errClinicUrl: "אנא הזן כתובת URL תקינה",
    errFirstName: "שם פרטי הוא שדה חובה",
    errLastName: "שם משפחה הוא שדה חובה",
    errEmail: "כתובת אימייל לא תקינה",
    errPhone: "חובה להזין 10 ספרות",
    errSelectManager: "אנא בחר מנהל מרפאה",
    createNew: "צור חדש",
    selectExisting: "בחר קיים",
    noManagers: "לא נמצאו מנהלי מרפאות.",
    editTitle: "עריכת מנהל המרפאה",
    deleteTitle: "מחיקת מנהל מרפאה",
    deleteDescription: "האם אתה בטוח שברצונך למחוק את {{name}}? לא ניתן לבטל פעולה זו.",
    saveChanges: "שמור שינויים",
    saveError: "שמירת השינויים נכשלה. אנא נסה שוב.",
    deleteError: "מחיקת המנהל נכשלה. אנא נסה שוב.",
    cancel: "ביטול",
    actions: "פעולות",
    selectClinic: "בחר מרפאה",
    selectClinicDesc: "למנהל זה יש מספר מרפאות. איזו מהן תרצה לצפות?",
  },

  // Doctors
  doctors: {
    description: "צפייה וניהול כל הרופאים במרפאה",
    searchPlaceholder: "חיפוש לפי שם, אימייל או טלפון...",
    addNew: "הוסף רופא חדש",
    registerTitle: "רישום רופא",
    registerButton: "הרשם",
    firstNameLabel: "שם פרטי",
    lastNameLabel: "שם משפחה",
    emailLabel: "אימייל",
    phoneNumberLabel: "מספר טלפון",
    firstNamePlaceholder: "",
    lastNamePlaceholder: "",
    emailPlaceholder: "",
    phoneNumberPlaceholder: "",
    errFirstName: "שם פרטי הוא שדה חובה",
    errLastName: "שם משפחה הוא שדה חובה",
    errEmail: "כתובת דוא\"ל לא תקינה",
    errPhone: "מספר הטלפון חייב להכיל בדיוק 10 ספרות",
    backToDirectory: "חזרה לרשימה",
    detailError: "טעינת פרטי הרופא נכשלה.",

    editTitle: "עריכת רופא",
    deleteTitle: "מחיקת רופא",
    deleteDescription: "האם אתה בטוח שברצונך למחוק את {{name}}? לא ניתן לבטל פעולה זו.",
    saveChanges: "שמור שינויים",
    cancel: "ביטול",
    saveError: "שמירת השינויים נכשלה. אנא נסה שוב.",
    deleteError: "מחיקת הרופא נכשלה. אנא נסה שוב.",
  },

  // Medications
  medications: {
    title: "תרופות",
    allMedications: "כל התרופות",
    searchPlaceholder: "חיפוש תרופה...",
    sort: "מיון",
    noData: "לא נמצאו תרופות מתאימות",
    addNew: "הוסף תרופה חדשה",
    addToClinic: "הוסף תרופה למרפאה",
    sortAZ: "א → ת",
    sortZA: "ת → א",
    sortForm: "לפי צורה",
    medNameLabel: "שם התרופה",
    medFormLabel: "צורה",
    medUnitLabel: "יחידה",
    medNamePlaceholder: "",
    dosage: "מינון",
    frequency: "תדירות",
    startDate: "התחלה",
    endDate: "סיום",
    registerButton: "הוסף",
    registerTitle: "הוסף תרופה חדשה",
    addToClinicTitle: "הוסף תרופה למרפאה",
    searchGlobal: "חיפוש בקטלוג הגלובלי...",
    confirm: "אישור",
    errMedName: "שם התרופה הוא שדה חובה",
    errMedForm: "הטופס נדרש",
    errMedUnit: "היחידה נדרשת",
    deleteConfirmTitle: "למחוק את התרופה?",
    deleteConfirmDesc: "פעולה זו תסיר את התרופה לצמיתות. לא ניתן לבטל פעולה זו.",
    deleteConfirm: "כן, מחק",
    freq: {
      ONCE: "פעם אחת",
      DAILY: "יומי",
      WEEKLY: "שבועי",
      MONTHLY: "חודשי",
    },
  },

  // Patient Medications
  patientMeds: {
    title: "התרופות שלי",
    addMedication: "הוספת תרופה",
    searchPlaceholder: "חיפוש תרופות...",
    standardDosage: "מינון סטנדרטי",
    intakeLogTitle: "יומן נטילת תרופות",
    refreshData: "רענון נתונים",
    regularDosage: "מינון קבוע",
    activeStatus: "פעיל",
    newPrescription: "מרשם חדש",
    configureSchedule: "הגדרת לוח זמנים ומינון לתרופה.",
    medicationLabel: "תרופה",
    timelineLabel: "טווח זמן",
    dosageLabel: "מינון",
    startDate: "תאריך התחלה",
    endDate: "תאריך סיום",
    amount: "כמות",
    unit: "יחידה",
    frequencyLabel: "תדירות ולוח זמנים",
    frequencyOptions: {
      once: "פעם אחת",
      daily: "יומי",
      weekly: "שבועי",
      monthly: "חודשי"
    },
    timesPerDay: "מספר פעמים ביום",
    addSlot: "הוספת שעה",
    finalize: "שיוך מרשם",
    editPrescription: "עריכת מרשם",
    saveChanges: "שמירת שינויים",
    addSuccess: "המרשם נוסף בהצלחה.",
    addError: "הוספת המרשם נכשלה. נסה שוב.",
    cancel: "ביטול",
    questions: {
      whichDays: "אילו ימים בשבוע?",
      whichWeeks: "אילו שבועות בחודש?"
    },
    units: {
      ml: "מ\"ל",
      mg: "מ\"ג",
      tabs: "טבליות"
    },
    days: {
      monday: "שני",
      tuesday: "שלישי",
      wednesday: "רביעי",
      thursday: "חמישי",
      friday: "שישי",
      saturday: "שבת",
      sunday: "ראשון"
    },
    weeks: {
      1: "שבוע ראשון",
      2: "שבוע שני",
      3: "שבוע שלישי",
      4: "שבוע רביעי"
    },
    deleteConfirm: {
      title: "האם אתה בטוח שברצונך למחוק תרופה זו?",
      description: "הפעולה תסיר את התרופה לצמיתות מרשומת המטופל.",
      cancel: "ביטול",
      confirm: "מחיקה"
    },
    resetFilters: "איפוס סינון",
    filters: {
      dateFrom: "מתאריך:",
      dateTo: "עד תאריך:",
      medication: "תרופה:",
      allMeds: "כל התרופות"
    },
    table: {
      intakeDate: "תאריך נטילה",
      intakeTime: "שעת נטילה",
      medId: "מזהה תרופה",
      medName: "שם התרופה",
      form: "צורת מתן",
      dosage: "מינון",
      status: "סטטוס",
      editSuccess: "עדכון המרשם בוצע בהצלחה.",
      editError: "עדכון המרשם נכשל.",
      deleteSuccess: "המרשם נמחק בהצלחה.",
      deleteError: "מחיקת המרשם נכשלה.",
    }
  },

  // Clinic
  clinic: {
    Title: "סקירת מרפאה",
    systemModules: "מודולים במערכת",
    noData: "אין נתוני מרפאה זמינים",
    managerTitle: "מנהל מרפאה",
    primaryAdmin: "אדמיניסטרטור ראשי",
    researchFacility: "מתקן מחקר",
    locationLabel: "מיקום הקליניקה",
    defaultName: "שם קליניקה",
    portalLabel: "פורטל הקליניקה",
    establishedLabel: "נוסד בתאריך",
    readOnlyView: ".תצוגת קריאה בלבד של תצורת הקליניקה הראשית",
    editTitle: "עריכת מרפאה",
    clinicNameLabel: "שם המרפאה",
    clinicUrlLabel: "כתובת המרפאה",
    clinicImageLabel: "כתובת תמונה",
    isResearchLabel: "מרפאת מחקר",
    managerLabel: "מנהל מרפאה",
    saveError: "עדכון המרפאה נכשל. אנא נסה שוב.",
    deleteTitle: "מחיקת מרפאה",
    deleteDescription: "מחיקת מרפאה זו תסיר לצמיתות את כל הנתונים הקשורים, כולל רופאים, מטופלים, מודולים וכל הרשומות המשויכות. לא ניתן לבטל פעולה זו.",
    deleteSuccess: "המרפאה נמחקה בהצלחה",
    deleteError: "מחיקת המרפאה נכשלה. אנא נסה שוב.",
  },

  // Activities 
  activities: {
    title: "פעילויות",
    addBtn: "הוספת פעילות חדשה",
    searchPlaceholder: "חיפוש פעילות...",
    allActivities: "כל הפעילויות",
    noActivities: "לא נמצאו פעילויות במרפאה זו",
    count_one: "פעילות אחת",
    count_other: "{{count}} פעילויות",
    deleteWarning: "פעולה זו תסיר את הפעילות מהמרפאה לצמיתות. לא ניתן לבטל פעולה זו.",
    detailsTitle: "פרטי הפעילות",
    descriptionLabel: "תיאור הפעילות",
    addDialogTitle: "הוספת פעילות למרפאה",
    activityNameLabel: "שם הפעילות",
    activityNamePlaceholder: "למשל: יוגה בוקר",
    activityDescriptionLabel: "תיאור",
    activityDescriptionPlaceholder: "תיאור קצר של הפעילות...",
    errActivityName: "שם הפעילות הוא שדה חובה",
    errActivityDescription: "תיאור הפעילות הוא שדה חובה",
    deleteGlobalWarning: "פעולה זו לא ניתנת לביטול. הפעילות תימחק לצמיתות מהמערכת.",
    addError: "הוספת הפעילות נכשלה. אנא נסה שנית.",
    deleteError: "מחיקת הפעילות נכשלה. אנא נסה שנית.",
  },

  // Patient Activities
  patientActivities: {
    title: "הפעילויות שלי",
    searchPlaceholder: "חיפוש פעילויות...",
    addActivity: {
      title: "הוספת פעילות",
      newActivity: "פעילות חדשה",
      timelineLabel: "ציר זמן",
      startDate: "תאריך התחלה",
      endDate: "תאריך סיום",
      frequencyLabel: "תדירות ולוח זמנים",
      frequencyOptions: {
        once: "פעם אחת",
        daily: "יומי",
        weekly: "שבועי",
        monthly: "חודשי",
      },
      daysOfWeek: "באילו ימים בשבוע?",
      dayOfMonth: "באיזה יום בחודש?",
      addTime: "הוסף שעה",
      days: {
        MON: "שני",
        TUE: "שלישי",
        WED: "רביעי",
        THU: "חמישי",
        FRI: "שישי",
        SAT: "שבת",
        SUN: "ראשון",
      },
      finalize: "שיוך פעילות",
      cancel: "ביטול",
      addSuccess: "הפעילות נוספה בהצלחה.",
      addError: "הוספת הפעילות נכשלה. נסה שוב.",
    },
    activityLabel: "פעילות",
    editActivity: {
      title: "עריכת פעילות",
      activityLabel: "פעילות",
      timelineLabel: "ציר זמן",
      startDate: "תאריך התחלה",
      endDate: "תאריך סיום",      
      editError: "עדכון הפעילות נכשל. נסה שוב.",
      editSuccess: "הפעילות עודכנה בהצלחה.",
      saveChanges: "שמור שינויים",
      cancel: "ביטול",
    },
    deleteConfirm: {
      title: "האם את/ה בטוח/ה שברצונך למחוק את הפעילות?",
      description: "הפעולה תמחק לצמיתות את הפעילות מרשומת המטופל.",
      cancel: "ביטול",
      confirm: "מחיקה",
    },
    logTitle: "יומן פעילויות",
    refreshData: "רענן נתונים",
    resetFilters: "איפוס סינון",
    defaultSubtitle: "פעילות כללית",
    table: {
      date: "תאריך",
      time: "שעה",
      activityName: "שם הפעילות",
    },
    filters: {
      allActivities: "כל הפעילויות",
      dateFrom: "מתאריך:",
      activity: "פעילות:",
    },
    deleteError: "מחיקת הפעילות נכשלה. נסה שוב.",
    deleteSuccess: "הפעילות נמחקה בהצלחה.",
  },

  // Patient Evaluations
  patientEvaluations: {
    questionnaires: "שאלונים",
    cognitiveTests: "מבחנים קוגניטיביים",
    moduleQuestionnaire: "שאלון מודול",
    medicalStaffEvaluation: "הערכה של צוות רפואי",
    noData: "אין נתונים זמינים",
    submissions: {
      backToEvaluations: "← חזרה למדדים",
      evaluationTitleFallback: "מדד",
      patientNameFallback: "מטופל",
      pageDescription: "סקירת נתונים היסטוריים או הגדרת מדדים חדשים.",
      settingsButton: "הגדרות מדדים למטופל",
      tableTitle: "שליחות מטופל",
      tableDescription: "סקירת נתונים ותוצאות שהוזנו על ידי המטופל.",
      loadError: "טעינת הנתונים נכשלה.",
      empty: "לא נמצאו נתונים.",
      columns: {
        submissionDate: "תאריך שליחה",
        frequency: "תדירות",
        grade: "ציון",
        actions: "פעולות",
      },
    },
    settings: {
      title: "הגדרת מדד",
      subtitle: "הגדרה עבור {{name}}",
      timelineLabel: "ציר זמן",
      startDate: "תאריך התחלה",
      endDate: "תאריך סיום (אופציונלי)",
      sendToPatient: "שלח למטופל",
      addTime: "הוסף שעה",
      dayOfMonth: "יום בחודש",
      daysOfWeek: "ימים בשבוע",
      success: "המדד הוקצה בהצלחה.",
      error: "הקצאת המדד נכשלה.",
    },
    deleteConfirm: {
      title: "האם אתה בטוח שברצונך למחוק מדד זה?",
      description: "פעולה זו תמחק את המדד לצמיתות מרשומת המטופל.",
      cancel: "ביטול",
      confirm: "מחק"
    },
    deleteSuccess: "המדד נמחק בהצלחה.",
    deleteError: "מחיקת המדד נכשלה. אנא נסה שוב.",
  },

  // Settings
  settings: {
    title: "הגדרות",
    appearance: "מראה",
    themes: {
      light: "בהיר",
      dark: "כהה",
      system: "מערכת",
    },
    language: "שפה",
    languageDesc: "בחר את השפה המועדפת עליך. הממשק יתעדכן מיד.",
    profile: {
      title: "פרופיל",
      phone: "טלפון",
      role: "תפקיד",
      accountStatus: "סטטוס חשבון",
      twoFA: "אימות דו-שלבי",
      memberSince: "חבר מאז",
      currentClinic: "מרפאה נוכחית",
      associatedClinics: "מרפאות משויכות",
      active: "פעיל",
      inactive: "לא פעיל",
      enabled: "מופעל",
      disabled: "מושבת",
      clinicCount: "{{count}} מרפאות",
      roles: {
        ADMIN: "מנהל מערכת",
        CLINIC_MANAGER: "מנהל מרפאה",
        DOCTOR: "רופא",
        PATIENT: "מטופל",
        RESEARCH_PATIENT: "מטופל מחקר",
      },
    },
  },

  // Not Found / 404
  notFound: {
    title: "404",
    subtitle: "הדף לא נמצא",
    back: "חזרה ללוח הבקרה",
  },

  // Error
  error: {
    title: "משהו השתבש",
    fallback: "אירעה שגיאה בלתי צפויה",
    back: "חזרה ללוח הבקרה",
  },

  // Common
  common: {
    toggleSidebar: "החלף סרגל צד",
    sessionExpired: "פג תוקף ההפעלה. אנא התחבר שוב.",
    edit: "עריכה",
    editClinic: "עריכת קליניקה",
    na: "לא זמין",
    cancel: "ביטול",
    delete: "מחיקה",
    areYouSure: "האם את בטוחה?",
    noDescription: "אין תיאור זמין לפעילות זו",
    confirm: "אישור",
    close: "סגירה",
    fetchingDetails: "מושך נתונים מהשרת...",
    upload_file: "העלה קובץ",
    system: "מערכת",
    save: "שמור",
    loading: {
      title: "מעבד את הבקשה שלך",
      fetchData: "אנא המתן בזמן שאנו אוספים את הנתונים.",
      buttonText: "אנא המתן..."
    }
  },
  
  // Modules
  modules: {
    title: "המודולים שלי",
    adminTitle: "מודולים",
    addModule: "הוסף מודול",
    editModule: "ערוך מודול",
    moduleNameLabel: "שם המודול",
    moduleNamePlaceholder: "למשל: לוח בקרה",
    descriptionLabel: "תיאור",
    descriptionPlaceholder: "תיאור קצר של המודול...",
    deleteTitle: "מחק מודול",
    deleteDescription: "האם אתה בטוח שברצונך למחוק את '{{name}}'? פעולה זו אינה הפיכה.",
    save: "שמור",
    cancel: "ביטול",
    errModuleName: "שם המודול הוא שדה חובה",
    searchPlaceholder: "חיפוש מודולים...",
    noData: "לא נמצאו מודולים.",
    columnName: "מודול",
    columnDescription: "תיאור",
    columnActions: "פעולות",
    clinicManagerTitle: "מודולי הקליניקה",
    addClinicModuleTitle: "הוספת מודולים לקליניקה",
    saveModules: "שמור",
    viewModule: "צפייה במודול",
    removeFromClinic: "הסר מהקליניקה",
    removeClinicModuleTitle: "הסרת מודול",
    removeClinicModuleDescription: "להסיר את '{{name}}' מהקליניקה? המטופלים יאבדו גישה.",
    saveError: "השמירה נכשלה. נסה שוב.",
    deleteError: "הסרת המודול נכשלה.",
    cards: {
      dashboard: {
        title: "לוח בקרה",
        description: "סקירת מדדים מרכזיים, התראות וסיכומי מטופלים.",
      },
      "document share": {
        title: "שיתוף מסמכים",
        description: "שיתוף וניהול מסמכים קליניים וקבצי מטופלים.",
      },
      evaluations: {
        title: "מדידות",
        description: "מעקב ורישום סימנים חיוניים ומדידות קליניות.",
      },
      chat: {
        title: "צ'אט",
        description: "הודעות מאובטחות בין רופאים ומטופלים.",
      },
      medications: {
        title: "תרופות",
        description: "ניהול מרשמים, מעקב מינונים, והתראות על רגישויות.",
      },
      activities: {
        title: "פעילויות",
        description: "תזמון תורנויות, סדנאות והדרכות צוות.",
      },
      memory: {
        title: "זיכרון",
        description: "תרגילי זיכרון קוגניטיביים וכלי הערכה.",
      },
      cdt: {
        title: "CDT",
        description: "מבחן ציור שעון לסינון קוגניטיבי.",
      },
      orientation: {
        title: "התמצאות",
        description: "הערכות התמצאות ומבחני מודעות מרחבית.",
      },
      hitber: {
        title: "HitBer",
        description: "הערכת תפקוד קוגניטיבי ומוטורי HitBer.",
      },
      tenstreatment: {
        title: "טיפול TENS",
        description: "ניהול טיפול בגירוי חשמלי עצבי דרך העור.",
      },
      parkinson: {
        title: "פרקינסון",
        description: "כלי ניטור וניהול מחלת פרקינסון.",
      },
      "parkinson report": {
        title: "דוח פרקינסון",
        description: "דוחות וניתוחים מפורטים למטופלי פרקינסון.",
      },
      "parkinson sensors": {
        title: "חיישני פרקינסון",
        description: "איסוף נתוני חיישנים להערכת תנועה בפרקינסון.",
      },
      analytics: {
        title: "אנליטיקה",
        description: "ניתוח נתונים, גרפים ומדדי ביצוע קליניים.",
      },
      settings: {
        title: "הגדרות",
        description: "הגדרת מודולים וניהול העדפות.",
      },
      "market test": {
        title: "מבחן שוק",
        description: "כלי בדיקות שוק ומסגרות הערכה.",
      },
      pass: {
        title: "כרטיס כניסה",
        description: "ניהול גישת מטופלים ותעודות אבטחה.",
      },
    },
  },
  // Patient Details
  patient:{
    editDetails: "עריכת פרטים",
    active: "פעיל",
    clinicPatient: "מטופל מרפאה",
    createdDate: "תאריך יצירה",
    lastLogin: "התחברות אחרונה",
    phone: "טלפון",
    email: "אימייל",
    customFunctions: "פונקציות מותאמות",
    metrics: "מדדים",
    modules: "מודולים",
    graphs: "גרפים",
    noData: "אין נתונים זמינים",
    editDialogTitle: "עריכת פרטי מטופל",
    firstName: "שם פרטי",
    lastName: "שם משפחה",
    saveChanges: "שמירת שינויים",
    cancel: "ביטול",
    enable: "הפעל",
  },

  // Graphs
  graphs: {
    title: "גרפים",
    createNew: "צור חדש",
    createTitle: "יצירת גרף",
    editTitle: "עריכת גרף",
    createSubtitle: "הגדר אילו נתונים להציג וכיצד למפות את הצירים.",
    editSubtitle: "עדכן את פרטי הגרף ואת תצורת הצירים.",
    basicInfoSection: "מידע בסיסי",
    axisSection: "תצורת צירים",
    patientTitle: "גרפים של המטופל",
    availableCharts: "גרפים זמינים",
    dateFilter: "תאריך",
    aboutChart: "אודות הגרף הזה",
    valueLegend: "משמעות הערכים",
    seriesToggle: "סדרות מוצגות",
    seriesUnnamed: "ללא שם",
    searchPlaceholder: "חיפוש גרפים...",
    empty: "עדיין לא הוגדרו גרפים.",
    nameLabel: "שם",
    namePlaceholder: "הזן שם גרף...",
    questionnaireLabel: "שאלון",
    selectQuestionnaire: "בחר שאלון...",
    descriptionLabel: "תיאור",
    descriptionPlaceholder: "תיאור (אופציונלי)...",
    noDescription: "אין תיאור",
    xAxisLabel: "ציר X",
    yAxisLabel: "ציר Y",
    axisLabelLabel: "תווית ציר",
    axisLabelPlaceholder: "לדוגמה: תאריך, ציון...",
    axisSource: {
      submissionDate: "תאריך הגשה",
      submissionScore: "ציון ההגשה",
      elementAnswer: "תשובה לשאלה",
    },
    selectElement: "בחר שאלה...",
    save: "שמור",
    edit: "ערוך",
    delete: "מחק",
    cancel: "ביטול",
    deleteTitle: "מחיקת גרף",
    deleteDescription: "האם אתה בטוח שברצונך למחוק את \"{{name}}\"? לא ניתן לבטל פעולה זו.",
    loading: "טוען גרף...",
    noData: "אין עדיין נתונים זמינים עבור מטופל זה.",
    createSuccess: "הגרף נוצר בהצלחה",
    createError: "יצירת הגרף נכשלה",
    updateSuccess: "הגרף עודכן בהצלחה",
    updateError: "עדכון הגרף נכשל",
    deleteSuccess: "הגרף נמחק בהצלחה",
    deleteError: "מחיקת הגרף נכשלה",
    errName: "השם חייב להכיל לפחות 2 תווים",
    errEvaluation: "אנא בחר שאלון",
    errChartType: "אנא בחר סוג גרף",
    errElement: "אנא בחר שאלה",
    errOverlayQuestion: "אנא בחר שאלון ושאלה עבור מקור נתונים זה",
    overlay: {
      section: "נתונים נוספים",
      hint: "הוסף נתונים משאלות או ממודולים אחרים שיוצגו על אותו גרף.",
      add: "הוסף מקור נתונים",
      remove: "הסר",
      typeQuestion: "שאלה אחרת",
      typeMedication: "תרופה שנלקחה",
      typeActivity: "פעילות שבוצעה",
      renderMarkers: "הצג כסמנים צבעוניים",
      renderLine: "הצג כקו",
      namePlaceholder: "סנן לפי שם (השאר ריק להכל)...",
      labelPlaceholder: "תווית תצוגה (אופציונלי)...",
      colorLabel: "צבע",
    },
  },

  // Analytics
analytics: {
  title: "אנליטיקה",
  subtitle: "לחץ על כרטיס כדי לצפות בסטטיסטיקות וגרפים מפורטים.",
  loading: "טוען ניתוח...",
  error: "טעינת הנתונים נכשלה. אנא נסה שוב.",
  cards: {
    activities: {
      title: "אנליטיקת פעילויות",
      description: "ניתוח שימוש בפעילויות, מגמות ונתוני מעורבות מטופלים."
    },
    medications: {
      title: "אנליטיקת תרופות",
      description: "ניתוח שימוש בתרופות, היענות לטיפול ומגמות קשורות."
    },
    evaluations: {
      title: "אנליטיקת מדדים",
      description: "סקירת תוצאות מדדים, הגשות ותגובות מטופלים."
    },
    file_share: {
    title: "שיתוף קבצים",
    description: "סטטיסטיקות ומעקב אחר העברות ושיתופי קבצים במערכת."
},
    parkinson_sensors: {
      title: "אנליטיקת חיישני פרקינסון",
      description: "ניתוח קריאות חיישנים ומגמות תנועה לניטור פרקינסון."
    }
  },
  activities: {
    title: "אנליטיקת פעילויות",
    description: "ניתוח שימוש בפעילויות, מגמות ונתוני מעורבות מטופלים.",
    busiestDaysChart: "ימים עמוסים ביותר",
    usageChart: "שימוש לאורך זמן"
  },
  medications: {
    title: "אנליטיקת תרופות",
    description: "ניתוח שימוש בתרופות, היענות לטיפול ומגמות קשורות.",
    busiestDaysChart: "ימים עמוסים ביותר",
    usageChart: "שימוש לאורך זמן"
  },
  evaluations: {
    title: "אנליטיקת מדדים",
    description: "סקירת תוצאות מדדים, הגשות ותגובות מטופלים.",
    busiestDaysChart: "ימים עמוסים ביותר",
    usageChart: "שימוש לאורך זמן",
    summary: {
    total: "סך הכל הגשות",
    mostUsed: "המדד הנפוץ ביותר",
    types: "סוגי מדדים פעילים",
    }
  },
  export: "ייצוא",
  filters: {
  startDate: "תאריך התחלה",
  endDate: "תאריך סיום",
},
},

  // Evaluations
  evaluations: {
    title: "מדדים",
    createNew: "יצירת חדש",
    searchPlaceholder: "חיפוש מדדים...",
    noData: "אין עדיין מדדים. צור את הראשון!",
    componentCount: "רכיבים",
    screenCount: "מסכים",
    lastModified: "עודכן לאחרונה",
    createdBy: "נוצר על ידי",
    edit: "עריכה",
    delete: "מחיקה",
    duplicate: "שכפול",
    preview: "תצוגה מקדימה",
    deleteTitle: "מחיקת מדד",
    deleteDescription: "האם אתה בטוח שברצונך למחוק את \"{{name}}\"? פעולה זו אינה ניתנת לביטול.",
    createTitle: "יצירת מדד",
    nameLabel: "שם",
    namePlaceholder: "הזן שם מדד...",
    create: "יצירה",
    cancel: "ביטול",
    errName: "השם חייב להכיל לפחות 2 תווים",
    createSuccess: "המדד נוצר בהצלחה",
    createError: "יצירת המדד נכשלה. נסה שוב.",
    deleteSuccess: "המדד נמחק בהצלחה",
    deleteError: "מחיקת המדד נכשלה. נסה שוב.",
    updateSuccess: "המדד עודכן בהצלחה",
    updateError: "עדכון המדד נכשל. נסה שוב.",
    editTitle: "עריכת מדד",
    isPublic: "ציבורי",
    addExisting: "הוספת קיים",
    addExistingTitle: "הוספת מדד קיים",
    noExistingData: "אין מדדים קיימים זמינים.",
    searchExisting: "חיפוש מדדים קיימים...",
    add: "הוסף",
    typeLabel: "סוג",
    errType: "יש לבחור סוג מדד",
    public: "ציבורי",
    active: "פעיל",
    adoptSuccess: "המדד אומץ בהצלחה",
    adoptError: "אימוץ המדד נכשל. נסה שוב.",
    selectAll: "בחר הכל",
    deselectAll: "בטל בחירה",
    addCount: "הוסף ({{count}})",
    adoptPartialError: "חלק מהמדדים לא נוספו",
    showAdopted: "הצג מדדים שאומצו",
    alreadyAdded: "כבר נוסף",
    emptyColumn: "אין מדדים בקטגוריה זו",

    results_title: "תוצאות הערכה",
    submitted_at: "נשלח בתאריך",
    list_view: "תצוגת רשימה",
    analytics_view: "תצוגת ניתוח",
    score: "ציון",
    error_title: "שגיאה: לא נמצאה הגשה",
    error_desc: "בדוק אם המזהים תקינים.",

    status: {
      excellent: "מצוין",
      pass: "עבר",
      fail: "נכשל",
    },

    analytics: {
      avg_grade: "ציון ממוצע",
      passed: "שאלות שעברו",
      needs_improvement: "דורש שיפור",
      questions: "שאלות",
      grade_distribution: "התפלגות ציונים",
      pass_fail_ratio: "יחס עבר/נכשל",
      passed_label: "אחוז מעבר",
      pass: "עבר",
      fail: "נכשל",
      count: "כמות",
    },

    table: {
      question_label: "שאלה",
      answer_provided: "תשובה שניתנה",
      status: "סטטוס",
      grade: "ציון",
      actions: "פעולות",
    },

    types: {
      questionnaries: "שאלונים",
      cognitiveTests: "מבחנים קוגניטיביים",
      moduleQuestionnaire: "שאלון מודול",
      medicalStaffEvaluation: "הערכה של צוות רפואי",
    },
    versionControl: "ניהול גרסאות",
    branchVersion: "צור גרסה חדשה",
    deleteVersion: "מחק גרסה",
    noVersions: "אין גרסאות עדיין",
    activeVersion: "פעיל",
    deleteVersionConfirm: "למחוק גרסה זו? הרכיב יחזור לגרסה אחרת.",
    selectVersion: "בחר גרסה",

    builder: {
      title: "בניית מסך",
      clearCanvas: "נקה קנבס",
      previewMode: "תצוגה מקדימה",
      exitPreview: "יציאה מתצוגה מקדימה",
      save: "שמור",
      saveSuccess: "המבנה נשמר בהצלחה",
      saveError: "שמירת המבנה נכשלה. נסה שוב.",
      back: "חזור",
      emptyCanvas: "גרור רכיבים מהפאנל השמאלי כדי להתחיל",
      noSelection: "בחר רכיב כדי לערוך את המאפיינים שלו",
      properties: "מאפיינים",
      screens: {
        addScreen: "הוסף מסך",
        deleteScreen: "מחק מסך",
        renameScreen: "שנה שם מסך",
        screenTitle: "מסך {{number}}",
        deleteConfirm: "האם אתה בטוח שברצונך למחוק את המסך?",
        cannotDeleteLast: "לא ניתן למחוק את המסך האחרון",
        screenIndicator: "מסך {{current}} מתוך {{total}}",
      },
      preview: {
        next: "הבא",
        previous: "הקודם",
        screenOf: "מסך {{current}} מתוך {{total}}",
      },
      totalScore: "ציון כולל",
    },

    categories: {
      textDisplay: "טקסט ותצוגה",
      userInputs: "קלט משתמש",
      actions: "פעולות",
      structure: "מבנה",
    },

    components: {
      heading: "כותרת",
      paragraph: "פסקה",
      infoCard: "מידע",
      image: "תמונה",
      icon: "אייקון",
      textInput: "טקסט",
      numberInput: "מספר",
      dropdown: "רשימה",
      multiSelect: "בחירה מרובה",
      radioGroup: "בחירה אחת",
      cardRadioGroup: "כרטיס בחירה",
      cardMultiSelect: "כרטיס מרובה",
      datePicker: "תאריך",
      timePicker: "שעה",
      scale: "סולם",
      toggleSwitch: "מתג",
      button: "כפתור",
      recordButton: "הקלטה",
      fileUpload: "העלאה",
      keyboard: "מקלדת",
      rowContainer: "שורה",
      visualQuestion: "שאלה ויזואלית",
    },

    props: {
      label: "תווית",
      text: "טקסט",
      placeholder: "טקסט עזר",
      required: "חובה",
      options: "אפשרויות",
      addOption: "הוסף אפשרות",
      min: "מינימום",
      max: "מקסימום",
      step: "שלב",
      level: "רמה",
      variant: "סוג",
      src: "קישור מקור",
      alt: "טקסט חלופי",
      iconName: "שם אייקון",
      size: "גודל",
      layout: "פריסה",
      defaultValue: "ערך ברירת מחדל",
      correctAnswerType: "סוג תשובה",
      correctAnswer: "תשובה נכונה",
      grade: "ציון",
      optionScore: "ניקוד",
      totalScore: "ציון כולל",
      minLabel: "תווית מינימום",
      maxLabel: "תווית מקסימום",
      trueLabel: "תווית נכון",
      falseLabel: "תווית לא נכון",
      allowPartialScore: "אפשר ניקוד חלקי",
      buttonLabel: "תווית כפתור",
      fileType: "סוג קובץ",
      keyboardType: "סוג מקלדת",
      visualKey: "מפתח ויזואלי",
      customKey: "מפתח מותאם",
      spots: "נקודות",
      addSpot: "הוסף נקודה",
      spotPoint: "מפתח נקודה",
      subitems: "פריטי משנה",
      addSubitem: "הוסף פריט משנה",
    },

    device: {
      mobile: "נייד",
      tablet: "טאבלט",
      desktop: "מחשב",
    },
    deleteConfirm: {
      title: "האם אתה בטוח שברצונך למחוק תשובה זו?",
      description: "פעולה זו תמחק את התשובה לצמיתות מההערכה.",
      cancel: "ביטול",
      confirm: "מחק"
    },
    editScore: {
      title: "עריכת ניקוד שאלה",
      description: "עדכן את הניקוד עבור תשובה זו.",
      placeholder: "הזן ניקוד",
      cancel: "ביטול",
      save: "שמור"
    },
  },
  
  // Files
  fileShare: {
    title: "שיתוף קבצים",
    description:
      "שתף ונהל מסמכים עם מטופלים. בחר מטופל כדי לצפות בקבצים שלו, להעלות ולהוריד אותם.",
    viewPatients: "צפייה במטופלים",
  },
  parkinsonSensors: {
    title: "חיישני פרקינסון",
    description:
      "איסוף וניתוח נתוני חיישני תנועה לבישים להערכת תסמינים מוטוריים של מחלת פרקינסון. אפליקציית הנייד קוראת ברציפות נתוני תאוצה, ג'ירוסקופ וצעדים כדי לזהות רעד, נפילות, כמעט-נפילות ומצבי תרופה (ON/OFF), ולאחר מכן מעלה את התוצאות לבדיקה קלינית. בחר מטופל כדי לצפות בנתוני החיישנים שלו.",
    viewPatients: "צפייה במטופלים",
    calc: {
      heading: "כיצד מתבצעים החישובים",
      intro:
        "מצב התרופה (ON/OFF) מוערך על ידי השוואת הפעילות הנוכחית של המטופל לקו הבסיס האישי שלו, הנבנה לאורך זמן על המכשיר.",
      metricsTitle: "1. חישוב מדדי פעילות",
      metricsBody:
        "לאורך חלון של 60 דגימות לפחות מחושבים עוצמת התאוצה הממוצעת, שונות התאוצה (סטיית תקן), עוצמת הג'ירוסקופ הממוצעת ותדירות הצעדים (שינוי צעדים לדקה).",
      baselineTitle: "2. חישוב קו בסיס",
      baselineBody:
        "המדדים האלה ממוצעים לאורך זמן, יחד עם סטיות התקן שלהם, לקו בסיס אישי הנשמר במסד נתונים מקומי מסוג Room. הכיול \"הושלם\" רק לאחר 30 דגימות לפחות הפרושות על פני 4 שעות לפחות (מחזור תרופה אחד).",
      classifyTitle: "3. סיווג מצב ON/OFF",
      classifyBody:
        "המסווג ממיר את המדדים הנוכחיים לציון משוקלל יחיד מול קו הבסיס וממפה אותו למצב ON, OFF או לא מוגדר.",
      classifyPoints: [
        "ציון תקן לכל ממד: (הערך הנוכחי − ממוצע קו הבסיס) / סטיית התקן של קו הבסיס. ערך חיובי משמעו פעיל יותר מהרגיל.",
        "ציון משוקלל מורכב: עוצמת תאוצה 0.3, תדירות צעדים 0.3, שונות תאוצה 0.2, פעילות ג'ירוסקופ 0.2.",
        "רצועת היסטרזיס (−0.2 עד +0.2): המצב מתהפך רק כאשר הציון חוצה את הרצועה, אחרת הוא נשאר במצב הקודם כדי למנוע ריצוד. ציון > 0.2 ← ON, < −0.2 ← OFF.",
        "ביטחון = המינימום של (|ציון| ÷ 2, 1).",
        "מחזיר \"לא מוגדר\" עד להשלמת הכיול.",
      ],
    },
    detectors: {
      heading: "אירועים שזוהו",
      colEvent: "אירוע",
      colSensors: "חיישנים עיקריים",
      colMethod: "שיטה מרכזית",
      tremor: {
        name: "רעד",
        sensors: "תאוצה לינארית + ג'ירוסקופ + צעדים + אור",
        method:
          "יחס עוצמת FFT בתחום 4–6.5 הרץ, התאמת תדר ג'ירוסקופ, עם שער משך של 8 שניות.",
      },
      fall: {
        name: "נפילה",
        sensors: "תאוצה גולמית + תאוצה לינארית + ג'ירוסקופ",
        method:
          "נפילה חופשית (< 3 מ'/ש\"ק²) ← פגיעה (> 15 מ'/ש\"ק²) ← 2.5 שניות של חוסר תנועה (ממוצע נמוך וסטיית תקן נמוכה).",
      },
      nearFall: {
        name: "כמעט-נפילה",
        sensors: "תאוצה לינארית + ג'ירוסקופ",
        method:
          "לפחות 3 שיאים > 14 מ'/ש\"ק² בתוספת ג'ירוסקופ > 3.5, ולאחר מכן חזרה לנורמלי תוך 3 שניות.",
      },
      onOff: {
        name: "מצב ON/OFF",
        sensors: "תאוצה + ג'ירוסקופ + צעדים",
        method:
          "ציוני תקן משוקללים מול קו הבסיס האישי של 4 שעות, עם רצועת היסטרזיס.",
      },
    },
    upload: {
      heading: "תהליך ההעלאה",
      body:
        "אירועים שזוהו (עם מאגרי התאוצה/הג'ירוסקופ הגולמיים ונתוני החיישנים המצרפיים) ממופים לבקשת שרת המתויגת במזהי המטופל והמרפאה, ומועלים דרך שירות החיישנים. למודול מצב ON/OFF יש DTO, ממפה ונתיב בקשת שרת משלו.",
    },
    patientView: {
      empty: "עדיין לא הועלו נתוני חיישנים.",
      error_title: "לא ניתן לטעון את נתוני החיישנים",
      error_desc: "אירעה שגיאה בעת שליפת נתוני החיישנים. נסה שוב.",
      table: {
        event: "אירוע",
        detectedAt: "זמן זיהוי",
        sensors: "חיישנים",
        details: "פרטים",
      },
    },
  },
  files: {
    title: "מסמכי מטופל",
    subtitle: "ניהול וצפייה בקבצים רפואיים משותפים",

    upload_button: "העלה מסמך",
    dialog_title: "העלאת מסמך חדש",
    dialog_desc: "בחר קובץ להעלאה לתיק המטופל",

    drag_drop: "גרור ושחרר קובץ כאן",
    supported_formats: "תמיכה ב־PDF, DOCX, CSV, PNG, JPG, SVG",
    browse: "בחר מהמחשב",

    choose_different: "בחר קובץ אחר",
    save_as: "שמור בשם",
    size: "{{size}} MB",

    search_placeholder: "חפש קבצים...",

    missing_patient_context: "אין מטופל נבחר",
    select_patient_hint: "בחר מטופל כדי לצפות במסמכים שלו",

    empty: {
      title: "אין מסמכים זמינים"
    },

    table: {
      name: "שם",
      date: "תאריך",
      owner: "בעלים",
      actions: "פעולות"
    }
  },
  // frequencyManager
frequencyManager: {
  frequencyLabel: "תדירות ולוח זמנים",
  addTime: "הוסף שעה",
  dayOfMonth: "יום בחודש",
  daysOfWeek: "ימים בשבוע",
  frequencyOptions: {
    once: "חד פעמי",
    daily: "יומי",
    weekly: "שבועי",
    monthly: "חודשי",
  },
  days: {
    SUN: "ראשון",
    MON: "שני",
    TUE: "שלישי",
    WED: "רביעי",
    THU: "חמישי",
    FRI: "שישי",
    SAT: "שבת",
  }
}

} as const;

export default he;