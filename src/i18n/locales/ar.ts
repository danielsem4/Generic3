const ar = {
  // Navigation & Sidebar
  nav: {
    dashboard: "لوحة التحكم",
    patients: "المرضى",
    clinics: "العيادات",
    modules: "الوحدات",
    medications: "الأدوية",
    settings: "الإعدادات",
    reports: "التقارير",
    doctors: "الأطباء",
    clinicManagers: "مديرو العيادات",
    manage: "إدارة",
    analytics: "التحليلات",
    clinicModules: "وحدات العيادة",
    pageNotReady: "صفحة {{title}} غير موجودة بعد!",
    logout: "تسجيل الخروج",
    logoutConfirmTitle: "هل أنت متأكد أنك تريد تسجيل الخروج؟",
    logoutConfirmDesc: "سيتم تسجيل خروجك وإعادة توجيهك إلى شاشة تسجيل الدخول.",
    cancel: "إلغاء",
    logoutConfirm: "نعم، تسجيل الخروج",
    hello: "مرحباً، {{name}}",
    clinic: "عيادة",
  },

  // Login
  login: {
    title: "تسجيل الدخول إلى حسابك",
    description: "أدخل بريدك الإلكتروني أدناه لتسجيل الدخول إلى حسابك",
    email: "البريد الإلكتروني",
    password: "كلمة المرور",
    placeholder: "m@example.com",
    forgotPassword: "نسيت كلمة المرور؟",
    rememberMe: "تذكرني",
    welcomeBack: "مرحباً بعودتك",
    submit: "تسجيل الدخول",
    submitting: "جارٍ تسجيل الدخول...",
    errorMissing: "يرجى ملء جميع الحقول.",
    errorFailed: "فشل تسجيل الدخول. يرجى التحقق من بياناتك والمحاولة مرة أخرى.",
    successTitle: "تم تسجيل الدخول بنجاح!",
    successDesc: "جارٍ إعادة التوجيه إلى الصفحة الرئيسية...",
    failedTitle: "فشل تسجيل الدخول",
    failedDesc: "يرجى التحقق من بياناتك.",
    selectClinic: "اختر عيادة",
    selectClinicDesc: "اختر العيادة التي تريد إدارتها في هذه الجلسة.",
    twoFaTitle: "المصادقة الثنائية",
    twoFaDesc: "أدخل الرمز المكون من 6 أرقام المرسل إلى بريدك الإلكتروني.",
    verifying: "جارٍ التحقق...",
    confirm: "تأكيد",
  },

  // Forgot Password
  forgotPassword: {
    title: "إعادة تعيين كلمة المرور",
    description: "أدخل بريدك الإلكتروني وسنرسل لك رمز التحقق.",
    emailLabel: "البريد الإلكتروني",
    emailPlaceholder: "m@example.com",
    sendCode: "إرسال الرمز",
    sending: "جارٍ الإرسال...",
    newPasswordTitle: "تعيين كلمة مرور جديدة",
    newPasswordDesc: "اختر كلمة مرور قوية لحسابك.",
    newPasswordLabel: "كلمة المرور الجديدة",
    newPasswordPlaceholder: "********",
    confirmPasswordLabel: "تأكيد كلمة المرور",
    confirmPasswordPlaceholder: "********",
    submit: "إعادة تعيين كلمة المرور",
    submitting: "جارٍ إعادة التعيين...",
    backToLogin: "العودة إلى تسجيل الدخول",
    successTitle: "تمت إعادة تعيين كلمة المرور!",
    successDesc: "يمكنك الآن تسجيل الدخول بكلمة المرور الجديدة.",
    errorEmailFailed: "تعذر إرسال الرمز إلى هذا البريد الإلكتروني. حاول مرة أخرى.",
    errorResetFailed: "فشل في إعادة تعيين كلمة المرور. حاول مرة أخرى.",
    errorCodeInvalid: "رمز غير صالح أو منتهي الصلاحية. حاول مرة أخرى.",
    errorCodeNotVerified: "يرجى التحقق من الرمز قبل تعيين كلمة مرور جديدة.",
    codeTitle: "أدخل رمز التحقق",
    codeDesc: "أدخل الرمز المكون من 6 أرقام الذي أرسلناه إلى بريدك الإلكتروني.",
    resend: "إعادة إرسال الرمز",
    resendSuccess: "تم إرسال رمز جديد إلى بريدك الإلكتروني.",
  },

  // Home / Dashboard
  home: {
    loading: "جارٍ تحميل البيانات...",
    error: "خطأ في تحميل البيانات",
    patients: "المرضى",
    clinics: "العيادات",
    modules: "الوحدات",
    medications: "الأدوية",
    clinicManagers: "مديرو العيادات",
    doctors: "الأطباء",
    evaluations: "التقييمات",
    latestUsers: "أحدث المستخدمين",
    updatedNow: "تم التحديث الآن.",
    firstName: "الاسم الأول",
    lastName: "اسم العائلة",
    email: "البريد الإلكتروني",
    phone: "رقم الهاتف",
    role: "الدور",
    view: "عرض",
    viewPatient: "عرض المريض",
    edit: "تعديل",
    delete: "حذف",
    noData: "لا تتوفر بيانات ملف تعريف المستخدم.",
    showing: "عرض {{count}} من أصل {{total}} إدخالات.",
  },

  // Patients
  patients: {
    title: "إدارة المرضى",
    description: "عرض وإدارة جميع سجلات المرضى في النظام",
    addNew: "إضافة مريض جديد",
    searchPlaceholder: "البحث بالاسم، البريد الإلكتروني، الهوية أو الهاتف...",
    tableTitle: "قائمة المرضى المسجلين",
    noResults: "لم يتم العثور على مرضى يطابقون بحثك.",
    columnName: "الاسم الكامل",
    columnEmail: "البريد الإلكتروني",
    columnPhone: "رقم الهاتف",
    columnStatus: "الحالة / الدور",
    columnActions: "إجراءات",
    loading: "جاري تحميل سجلات المرضى...",
    columnId: "رقم الهوية",
    columnLastVisit: "آخر زيارة",
    statusActive: "نشط",
    statusPending: "قيد المراجعة",
    statusFollowUp: "قيد المتابعة",
    view: "عرض التفاصيل",
    errFirstName: "الاسم الأول حقل مطلوب",
    errLastName: "اسم العائلة حقل مطلوب",
    errEmail: "البريد الإلكتروني غير صحيح",
    errPhone: "يجب إدخال 10 أرقام",
    errPassMin: "يجب أن تتكون كلمة المرور من 6 أحرف على الأقل",
    errPassMatch: "كلمات المرور غير متطابقة",
    errPassUpper: "يجب أن تحتوي على حرف كبير واحد على الأقل",
    errPassNumber: "يجب أن تحتوي على رقم واحد على الأقل",
    errPassSpecial: "يجب أن تحتوي على حرف خاص واحد على الأقل (!@#$%^&*)",
    togglePatient: "مريض",
    toggleResearch: "مريض بحثي",
    registerTitle: "التسجيل",
    registerButton: "تسجيل",
    firstNameLabel: "الاسم الأول",
    lastNameLabel: "اسم العائلة",
    emailLabel: "البريد الإلكتروني",
    phoneNumberLabel: "رقم الهاتف",
    passwordLabel: "كلمة المرور",
    confirmPasswordLabel: "تأكيد كلمة المرور",
    firstNamePlaceholder: "",
    lastNamePlaceholder: "",
    emailPlaceholder: "",
    phoneNumberPlaceholder: "",
    passwordPlaceholder: "********",
    confirmPasswordPlaceholder: "********",
    save: "حفظ",
    patientAlreadyExistsError: "هذا المريض موجود بالفعل أو أن التفاصيل غير صحيحة",
    patientAddedSuccess: "تم إضافة المريض بنجاح",

    editTitle: "تعديل المريض",
    deleteTitle: "حذف المريض",
    deleteDescription: "هل أنت متأكد أنك تريد حذف {{name}}؟ لا يمكن التراجع عن هذا.",
    saveChanges: "حفظ التغييرات",
    cancel: "إلغاء",
    saveError: "فشل حفظ التغييرات. يرجى المحاولة مرة أخرى.",
    deleteError: "فشل حذف المريض. يرجى المحاولة مرة أخرى.",
  },

  // Clinic Managers
  clinicManagers: {
    description: "عرض وإدارة جميع مديري العيادات في النظام",
    searchPlaceholder: "البحث بالاسم أو البريد الإلكتروني أو الهاتف...",
    addNew: "إضافة عيادة جديدة",
    registerTitle: "إنشاء عيادة جديدة",
    registerButton: "إنشاء",
    back: "رجوع",
    next: "التالي",
    creating: "جارٍ الإنشاء...",
    clinicNameLabel: "اسم العيادة",
    clinicNamePlaceholder: "",
    clinicUrlLabel: "رابط العيادة",
    clinicUrlPlaceholder: "https://www.yourclinic.com",
    clinicImageUrlLabel: "رابط صورة العيادة",
    clinicImageUrlPlaceholder: "https://example.com/logo.png",
    clinicTypeLabel: "نوع العيادة",
    typeSTANDARD: "قياسية",
    typeRESEARCH: "بحثية",
    modulesSearchPlaceholder: "البحث عن وحدات...",
    modulesSelected: "{{count}} وحدات مختارة",
    modulesLoadError: "فشل تحميل الوحدات. يرجى المحاولة مرة أخرى.",
    managerFirstNameLabel: "الاسم الأول",
    managerLastNameLabel: "اسم العائلة",
    managerEmailLabel: "البريد الإلكتروني",
    managerPhoneLabel: "رقم الهاتف",
    managerPasswordLabel: "كلمة المرور",
    managerFirstNamePlaceholder: "",
    managerLastNamePlaceholder: "",
    managerEmailPlaceholder: "",
    managerPhonePlaceholder: "",
    managerPasswordPlaceholder: "",
    errPassword: "يجب أن تكون كلمة المرور 8 أحرف على الأقل",
    errImageUrl: "الرجاء إدخال رابط صالح",
    step1: "تفاصيل العيادة",
    step2: "وحدات العيادة",
    step3: "مدير العيادة",
    successTitle: "تم إنشاء العيادة!",
    successDesc: "تم إنشاء العيادة ومديرها بنجاح.",
    successClose: "إغلاق",
    errClinicName: "يجب أن يحتوي اسم العيادة على حرفين على الأقل",
    errClinicUrl: "يرجى إدخال رابط URL صحيح",
    errFirstName: "الاسم الأول حقل مطلوب",
    errLastName: "اسم العائلة حقل مطلوب",
    errEmail: "البريد الإلكتروني غير صحيح",
    errPhone: "يجب إدخال 10 أرقام",
    errSelectManager: "يرجى اختيار مدير عيادة",
    createNew: "إنشاء جديد",
    selectExisting: "اختيار موجود",
    noManagers: "لم يتم العثور على مديري عيادات.",
    editTitle: "تعديل مدير العيادة",
    deleteTitle: "حذف مدير العيادة",
    deleteDescription: "هل أنت متأكد أنك تريد حذف {{name}}؟ لا يمكن التراجع عن هذا.",
    saveChanges: "حفظ التغييرات",
    saveError: "فشل حفظ التغييرات. يرجى المحاولة مرة أخرى.",
    deleteError: "فشل حذف المدير. يرجى المحاولة مرة أخرى.",
    cancel: "إلغاء",
    actions: "الإجراءات",
    selectClinic: "اختر العيادة",
    selectClinicDesc: "هذا المدير لديه عدة عيادات. أيها تريد عرضه؟",
  },

  // Doctors
  doctors: {
    description: "عرض وإدارة جميع الأطباء في العيادة",
    searchPlaceholder: "البحث بالاسم أو البريد الإلكتروني أو الهاتف...",
    addNew: "إضافة طبيب جديد",
    registerTitle: "تسجيل طبيب",
    registerButton: "تسجيل",
    firstNameLabel: "الاسم الأول",
    lastNameLabel: "اسم العائلة",
    emailLabel: "البريد الإلكتروني",
    phoneNumberLabel: "رقم الهاتف",
    firstNamePlaceholder: "",
    lastNamePlaceholder: "",
    emailPlaceholder: "",
    phoneNumberPlaceholder: "",
    errFirstName: "الاسم الأول مطلوب",
    errLastName: "اسم العائلة مطلوب",
    errEmail: "عنوان البريد الإلكتروني غير صالح",
    errPhone: "يجب أن يتكون رقم الهاتف من 10 أرقام بالضبط",
    backToDirectory: "العودة إلى الدليل",
    detailError: "فشل في تحميل بيانات الطبيب.",

    editTitle: "تعديل الطبيب",
    deleteTitle: "حذف الطبيب",
    deleteDescription: "هل أنت متأكد أنك تريد حذف {{name}}؟ لا يمكن التراجع عن هذا.",
    saveChanges: "حفظ التغييرات",
    cancel: "إلغاء",
    saveError: "فشل حفظ التغييرات. يرجى المحاولة مرة أخرى.",
    deleteError: "فشل حذف الطبيب. يرجى المحاولة مرة أخرى.",
  },

  // Medications
  medications: {
    title: "الأدوية",
    allMedications: "جميع الأدوية",
    searchPlaceholder: "البحث عن دواء...",
    sort: "فرز",
    noData: "لم يتم العثور على أدوية مطابقة",
    addNew: "إضافة دواء جديد",
    addToClinic: "إضافة دواء إلى العيادة",
    sortAZ: "أ → ي",
    sortZA: "ي → أ",
    sortForm: "حسب الشكل",
    medNameLabel: "اسم الدواء",
    medFormLabel: "الشكل",
    medUnitLabel: "الوحدة",
    medNamePlaceholder: "",
    dosage: "الجرعة",
    frequency: "التكرار",
    startDate: "البداية",
    endDate: "النهاية",
    registerButton: "إضافة",
    registerTitle: "إضافة دواء جديد",
    addToClinicTitle: "إضافة دواء إلى العيادة",
    searchGlobal: "البحث في الكتالوج العالمي...",
    confirm: "تأكيد",
    errMedName: "اسم الدواء مطلوب",
    errMedForm: "النموذج مطلوب",
    errMedUnit: "الوحدة مطلوبة",
    deleteConfirmTitle: "حذف الدواء؟",
    deleteConfirmDesc: "سيؤدي هذا إلى إزالة الدواء نهائياً. لا يمكن التراجع عن هذا الإجراء.",
    deleteConfirm: "نعم، احذف",
    freq: {
      ONCE: "مرة واحدة",
      DAILY: "يومياً",
      WEEKLY: "أسبوعياً",
      MONTHLY: "شهرياً",
    },
  },

  // patient medications
  patientMeds: {
    title: "أدويتي",
    addMedication: "إضافة دواء",
    searchPlaceholder: "البحث عن الأدوية...",
    standardDosage: "الجرعة القياسية",
    intakeLogTitle: "سجل تناول الأدوية",
    refreshData: "تحديث البيانات",
    regularDosage: "الجرعة المنتظمة",
    activeStatus: "نشط",
    newPrescription: "وصفة جديدة",
    configureSchedule: "تحديد جدول وجرعة الدواء.",
    medicationLabel: "الدواء",
    timelineLabel: "الفترة الزمنية",
    dosageLabel: "الجرعة",
    startDate: "تاريخ البدء",
    endDate: "تاريخ الانتهاء",
    amount: "الكمية",
    unit: "الوحدة",
    frequencyLabel: "التكرار والجدول الزمني",
    frequencyOptions: {
      once: "مرة واحدة",
      daily: "يومي",
      weekly: "أسبوعي",
      monthly: "شهري"
    },
    timesPerDay: "عدد المرات يومياً",
    addSlot: "إضافة وقت",
    finalize: "تعيين الوصفة",
    editPrescription: "تعديل الوصفة",
    saveChanges: "حفظ التغييرات",
    addSuccess: "تمت إضافة الوصفة بنجاح.",
    addError: "فشل في إضافة الوصفة. حاول مرة أخرى.",
    cancel: "إلغاء",
    questions: {
      whichDays: "في أي أيام من الأسبوع؟",
      whichWeeks: "في أي أسابيع من الشهر؟"
    },
    units: {
      ml: "مل",
      mg: "ملغ",
      tabs: "أقراص"
    },
    days: {
      monday: "الاثنين",
      tuesday: "الثلاثاء",
      wednesday: "الأربعاء",
      thursday: "الخميس",
      friday: "الجمعة",
      saturday: "السبت",
      sunday: "الأحد"
    },
    weeks: {
      1: "الأسبوع الأول",
      2: "الأسبوع الثاني",
      3: "الأسبوع الثالث",
      4: "الأسبوع الرابع"
    },
    deleteConfirm: {
      title: "هل أنت متأكد أنك تريد حذف هذا الدواء؟",
      description: "سيؤدي هذا إلى إزالة الدواء نهائيًا من سجل المريض.",
      cancel: "إلغاء",
      confirm: "حذف"
    },
    resetFilters: "إعادة تعيين الفلاتر",
    filters: {
      dateFrom: "من تاريخ:",
      dateTo: "إلى تاريخ:",
      medication: "الدواء:",
      allMeds: "جميع الأدوية"
    },
    table: {
      intakeDate: "تاريخ التناول",
      intakeTime: "وقت التناول",
      medId: "معرّف الدواء",
      medName: "اسم الدواء",
      form: "الشكل",
      dosage: "الجرعة",
      status: "الحالة"
    },
    editSuccess: "تم تحديث الوصفة بنجاح.",
    editError: "فشل في تحديث الوصفة.",
    deleteSuccess: "تم حذف الوصفة بنجاح.",
    deleteError: "فشل في حذف الوصفة.",
  },

  // Clinic 
  clinic: {
    Title: "نظرة عامة على العيادة",
    systemModules: "وحدات النظام",
    noData: "لا توجد بيانات عيادة متاحة",
    managerTitle: "مدير العيادة",
    primaryAdmin: "المسؤول الرئيسي",
    editClinic: "تعديل تفاصيل العيادة",
    saveSuccess: "تم تحديث تفاصيل العيادة بنجاح",
    researchFacility: "مرفق أبحاث",
    locationLabel: "موقع العيادة",
    defaultName: "اسم العيادة",
    portalLabel: "بوابة العيادة",
    establishedLabel: "تأسست في",
    readOnlyView: "عرض للقراءة فقط لتكوين العيادة الأساسي.",
    editTitle: "تعديل العيادة",
    clinicNameLabel: "اسم العيادة",
    clinicUrlLabel: "رابط العيادة",
    clinicImageLabel: "رابط الصورة",
    isResearchLabel: "عيادة بحثية",
    managerLabel: "مدير العيادة",
    saveError: "فشل تحديث العيادة. يرجى المحاولة مرة أخرى.",
    deleteTitle: "حذف العيادة",
    deleteDescription: "سيؤدي حذف هذه العيادة إلى إزالة جميع البيانات المرتبطة بها نهائيًا، بما في ذلك الأطباء والمرضى والوحدات وجميع السجلات المرتبطة. لا يمكن التراجع عن هذا الإجراء.",
    deleteSuccess: "تم حذف العيادة بنجاح",
    deleteError: "فشل حذف العيادة. يرجى المحاولة مرة أخرى.",
  },

  // Activities 
  activities: {
    title: "الأنشطة",
    addBtn: "إضافة نشاط جديد",
    searchPlaceholder: "البحث عن نشاط...",
    allActivities: "جميع الأنشطة",
    noActivities: "لم يتم العثور على أنشطة في هذه العيادة",
    deleteWarning: "لا يمكن التراجع عن هذا الإجراء. سيؤدي هذا إلى حذف النشاط نهائيًا من العيادة.",
    count_one: "نشاط واحد",
    count_other: "{{count}} أنشطة",
    detailsTitle: "تفاصيل النشاط",
    descriptionLabel: "وصف النشاط",
    addDialogTitle: "إضافة نشاط للعيادة",
    activityNameLabel: "اسم النشاط",
    activityNamePlaceholder: "مثال: يوغا صباحية",
    activityDescriptionLabel: "الوصف",
    activityDescriptionPlaceholder: "وصف مختصر للنشاط...",
    errActivityName: "اسم النشاط مطلوب",
    errActivityDescription: "وصف النشاط مطلوب",
    deleteGlobalWarning: "لا يمكن التراجع عن هذا الإجراء. سيؤدي هذا إلى حذف النشاط نهائيًا من النظام.",
    addError: "فشل إضافة النشاط. يرجى المحاولة مرة أخرى.",
    deleteError: "فشل حذف النشاط. يرجى المحاولة مرة أخرى.",
  },

  //patient activities
  patientActivities: {
    title: "أنشطتي",
    searchPlaceholder: "ابحث عن الأنشطة...",
    addActivity: {
      title: "إضافة نشاط",
      newActivity: "نشاط جديد",
      timelineLabel: "الجدول الزمني",
      startDate: "تاريخ البدء",
      endDate: "تاريخ الانتهاء",
      frequencyLabel: "التكرار والجدول",
      frequencyOptions: {
        once: "مرة واحدة",
        daily: "يومي",
        weekly: "أسبوعي",
        monthly: "شهري",
      },
      daysOfWeek: "أي أيام في الأسبوع؟",
      dayOfMonth: "أي يوم في الشهر؟",
      addTime: "إضافة وقت",
      days: {
        MON: "الإثنين",
        TUE: "الثلاثاء",
        WED: "الأربعاء",
        THU: "الخميس",
        FRI: "الجمعة",
        SAT: "السبت",
        SUN: "الأحد",
      },
      finalize: "تعيين النشاط",
      cancel: "إلغاء",
      addSuccess: "تمت إضافة النشاط بنجاح.",
      addError: "فشل في إضافة النشاط. حاول مرة أخرى.",
    },
    activityLabel: "النشاط",
    editActivity: {
      title: "تعديل النشاط",
      activityLabel: "النشاط",
      timelineLabel: "الجدول الزمني",
      startDate: "تاريخ البدء",
      endDate: "تاريخ الانتهاء",
      editError: "فشل في تحديث النشاط. حاول مرة أخرى.",
      editSuccess: "تم تحديث النشاط بنجاح.",
      saveChanges: "حفظ التغييرات",
      cancel: "إلغاء",
    },
    deleteConfirm: {
      title: "هل أنت متأكد أنك تريد حذف هذا النشاط؟",
      description: "سيؤدي هذا إلى حذف النشاط نهائيًا من سجل المريض.",
      cancel: "إلغاء",
      confirm: "حذف",
    },
    logTitle: "سجل الأنشطة",
    refreshData: "تحديث البيانات",
    resetFilters: "إعادة تعيين الفلاتر",
    defaultSubtitle: "نشاط عام",
    table: {
      date: "التاريخ",
      time: "الوقت",
      activityName: "اسم النشاط",
    },
    filters: {
      allActivities: "كل الأنشطة",
      dateFrom: "من تاريخ:",
      activity: "النشاط:",
    },
    deleteError: "فشل في حذف النشاط. حاول مرة أخرى.",
    deleteSuccess: "تم حذف النشاط بنجاح.",
  },
  
  // patient evaluations
  patientEvaluations: {
    questionnaires: "استبيانات",
    cognitiveTests: "اختبارات معرفية",
    moduleQuestionnaire: "استبيان الوحدة",
    medicalStaffEvaluation: "تقييم الطاقم الطبي",
    noData: "لا توجد بيانات",
    submissions: {
      backToEvaluations: "← العودة إلى القياسات",
      evaluationTitleFallback: "قياس",
      patientNameFallback: "مريض",
      pageDescription: "عرض البيانات السابقة أو إعداد قياسات جديدة.",
      settingsButton: "إعدادات قياسات المريض",
      tableTitle: "إرسالات المريض",
      tableDescription: "عرض البيانات والنتائج التي أدخلها المريض.",
      loadError: "فشل في تحميل البيانات.",
      empty: "لا توجد بيانات.",
      columns: {
        submissionDate: "تاريخ الإرسال",
        frequency: "التكرار",
        grade: "الدرجة",
        actions: "إجراءات",
      },
    },
    settings: {
      title: "إعداد القياس",
      subtitle: "إعداد لـ {{name}}",
      timelineLabel: "الجدول الزمني",
      startDate: "تاريخ البدء",
      endDate: "تاريخ الانتهاء (اختياري)",
      sendToPatient: "إرسال إلى المريض",
      addTime: "إضافة وقت",
      dayOfMonth: "يوم في الشهر",
      daysOfWeek: "أيام الأسبوع",
      success: "تم تعيين القياس بنجاح.",
      error: "فشل في تعيين القياس.",
    },
    deleteConfirm: {
      title: "هل أنت متأكد أنك تريد حذف هذا القياس؟",
      description: "سيؤدي هذا إلى حذف القياس نهائيًا من سجل المريض.",
      cancel: "إلغاء",
      confirm: "حذف"
    },
    deleteSuccess: "تم حذف القياس بنجاح.",
    deleteError: "فشل في حذف القياس. حاول مرة أخرى.",
  },

  // Settings
  settings: {
    title: "الإعدادات",
    appearance: "المظهر",
    themes: {
      light: "فاتح",
      dark: "داكن",
      system: "النظام",
    },
    language: "اللغة",
    languageDesc: "اختر لغتك المفضلة. سيتم تحديث الواجهة فوراً.",
    profile: {
      title: "الملف الشخصي",
      phone: "الهاتف",
      role: "الدور",
      accountStatus: "حالة الحساب",
      twoFA: "المصادقة الثنائية",
      memberSince: "عضو منذ",
      currentClinic: "العيادة الحالية",
      associatedClinics: "العيادات المرتبطة",
      active: "نشط",
      inactive: "غير نشط",
      enabled: "مفعّل",
      disabled: "معطّل",
      clinicCount: "{{count}} عيادة/عيادات",
      roles: {
        ADMIN: "مسؤول",
        CLINIC_MANAGER: "مدير العيادة",
        DOCTOR: "طبيب",
        PATIENT: "مريض",
        RESEARCH_PATIENT: "مريض بحثي",
      },
    },
  },

  // Not Found / 404
  notFound: {
    title: "404",
    subtitle: "الصفحة غير موجودة",
    back: "العودة إلى لوحة التحكم",
  },

  // Error
  error: {
    title: "حدث خطأ ما",
    fallback: "حدث خطأ غير متوقع",
    back: "العودة إلى لوحة التحكم",
  },

  // Common
  common: {
    toggleSidebar: "تبديل الشريط الجانبي",
    sessionExpired: "انتهت الجلسة. يرجى تسجيل الدخول مرة أخرى.",
    edit: "تعديل",
    editClinic: "تعديل العيادة",
    na: "غير متاح",
    cancel: "إلغاء",
    delete: "حذف",
    areYouSure: "هل أنت متأكد؟",
    noDescription: "لا يوجد وصف متاح لهذا النشاط",
    confirm: "تأكيد",
    close: "إغلاق",
    fetchingDetails: "جاري جلب البيانات من الخادم...",
    upload_file: "تحميل ملف",
    system: "النظام",
    loading: {
      title: "معالجة طلبك",
      fetchData: "يرجى الانتظار بينما نجمع التفاصيل.",
      buttonText: "يرجى الانتظار..."
}
  },

  // Modules
  modules: {
    title: "وحداتي",
    adminTitle: "الوحدات",
    addModule: "إضافة وحدة",
    editModule: "تعديل الوحدة",
    moduleNameLabel: "اسم الوحدة",
    moduleNamePlaceholder: "مثال: لوحة التحكم",
    descriptionLabel: "الوصف",
    descriptionPlaceholder: "وصف مختصر للوحدة...",
    deleteTitle: "حذف الوحدة",
    deleteDescription: "هل أنت متأكد أنك تريد حذف '{{name}}'؟ لا يمكن التراجع عن هذا الإجراء.",
    save: "حفظ",
    cancel: "إلغاء",
    errModuleName: "اسم الوحدة مطلوب",
    searchPlaceholder: "البحث في الوحدات...",
    noData: "لم يتم العثور على وحدات.",
    columnName: "الوحدة",
    columnDescription: "الوصف",
    columnActions: "الإجراءات",
    clinicManagerTitle: "وحدات العيادة",
    addClinicModuleTitle: "إضافة وحدات إلى العيادة",
    saveModules: "حفظ",
    viewModule: "عرض الوحدة",
    removeFromClinic: "إزالة من العيادة",
    removeClinicModuleTitle: "إزالة الوحدة",
    removeClinicModuleDescription: "إزالة '{{name}}' من هذه العيادة؟ سيفقد المرضى الوصول.",
    saveError: "فشل الحفظ. يرجى المحاولة مرة أخرى.",
    deleteError: "فشل إزالة الوحدة.",
    cards: {
      dashboard: {
        title: "لوحة التحكم",
        description: "نظرة عامة على المقاييس الرئيسية والتنبيهات وملخصات المرضى.",
      },
      "document share": {
        title: "مشاركة المستندات",
        description: "مشاركة وإدارة المستندات السريرية وملفات المرضى.",
      },
      evaluations: {
        title: "القياسات",
        description: "تتبع وتسجيل العلامات الحيوية والقياسات السريرية.",
      },
      chat: {
        title: "المحادثة",
        description: "مراسلة آمنة بين الأطباء والمرضى.",
      },
      medications: {
        title: "الأدوية",
        description: "إدارة الوصفات، متابعة الجرعات، وتنبيهات الحساسية.",
      },
      activities: {
        title: "الأنشطة",
        description: "جدولة المناوبات، ورش عمل، وتتدريب الطاقم.",
      },
      memory: {
        title: "الذاكرة",
        description: "تمارين الذاكرة المعرفية وأدوات التقييم.",
      },
      cdt: {
        title: "CDT",
        description: "اختبار رسم الساعة للفحص المعرفي.",
      },
      orientation: {
        title: "التوجيه",
        description: "تقييمات التوجيه واختبارات الوعي المكاني.",
      },
      hitber: {
        title: "HitBer",
        description: "تقييم الوظائف المعرفية والحركية HitBer.",
      },
      tenstreatment: {
        title: "علاج TENS",
        description: "إدارة العلاج بالتحفيز الكهربائي للأعصاب عبر الجلد.",
      },
      parkinson: {
        title: "باركنسون",
        description: "أدوات مراقبة وإدارة مرض باركنسون.",
      },
      "parkinson report": {
        title: "تقرير باركنسون",
        description: "تقارير وتحليلات مفصلة لمرضى باركنسون.",
      },
      "parkinson sensors": {
        title: "مستشعرات باركنسون",
        description: "جمع بيانات المستشعرات لتقييم الحركة عند مرضى باركنسون.",
      },
      analytics: {
        title: "التحليلات",
        description: "تحليلات البيانات والرسوم البيانية ومقاييس الأداء السريري.",
      },
      settings: {
        title: "الإعدادات",
        description: "إعداد الوحدات وإدارة التفضيلات.",
      },
      "market test": {
        title: "اختبار السوق",
        description: "أدوات اختبار السوق وأُطر التقييم.",
      },
      pass: {
        title: "تصريح",
        description: "إدارة وصول المرضى وتصاريح الأمان.",
      },
    },
  },
  
  // Patient Details
  patient: {
    editDetails: "تعديل التفاصيل",
    active: "نشط",
    clinicPatient: "مريض العيادة",
    createdDate: "تاريخ الإنشاء",
    lastLogin: "آخر تسجيل دخول",
    phone: "الهاتف",
    email: "البريد الإلكتروني",
    customFunctions: "وظائف مخصصة",
    metrics: "المقاييس",
    modules: "الوحدات",
    graphs: "الرسوم البيانية",
    noData: "لا توجد بيانات متاحة",
    editDialogTitle: "تعديل تفاصيل المريض",
    firstName: "الاسم الأول",
    lastName: "اسم العائلة",
    saveChanges: "حفظ التغييرات",
    cancel: "إلغاء",
    enable: "تفعيل",
  },

  // Graphs
  graphs: {
    title: "الرسوم البيانية",
    createNew: "إنشاء جديد",
    createTitle: "إنشاء رسم بياني",
    editTitle: "تعديل الرسم البياني",
    createSubtitle: "حدد البيانات المراد رسمها وكيفية ربط المحاور.",
    editSubtitle: "قم بتحديث تفاصيل الرسم البياني وإعدادات المحاور.",
    basicInfoSection: "المعلومات الأساسية",
    axisSection: "إعدادات المحاور",
    patientTitle: "الرسوم البيانية للمريض",
    availableCharts: "الرسوم البيانية المتاحة",
    dateFilter: "التاريخ",
    aboutChart: "حول هذا الرسم البياني",
    valueLegend: "ماذا تعني القيم",
    seriesToggle: "السلاسل المرئية",
    seriesUnnamed: "بدون اسم",
    searchPlaceholder: "البحث في الرسوم البيانية...",
    empty: "لم يتم تكوين أي رسوم بيانية بعد.",
    nameLabel: "الاسم",
    namePlaceholder: "أدخل اسم الرسم البياني...",
    questionnaireLabel: "الاستبيان",
    selectQuestionnaire: "اختر استبيانًا...",
    descriptionLabel: "الوصف",
    descriptionPlaceholder: "وصف اختياري...",
    noDescription: "لا يوجد وصف",
    xAxisLabel: "المحور السيني",
    yAxisLabel: "المحور الصادي",
    axisLabelLabel: "تسمية المحور",
    axisLabelPlaceholder: "مثال: التاريخ، النتيجة...",
    axisSource: {
      submissionDate: "تاريخ التقديم",
      submissionScore: "نتيجة التقديم",
      elementAnswer: "إجابة السؤال",
    },
    selectElement: "اختر سؤالًا...",
    save: "حفظ",
    edit: "تعديل",
    delete: "حذف",
    cancel: "إلغاء",
    deleteTitle: "حذف الرسم البياني",
    deleteDescription: "هل أنت متأكد أنك تريد حذف \"{{name}}\"؟ لا يمكن التراجع عن هذا الإجراء.",
    loading: "جارٍ تحميل الرسم البياني...",
    noData: "لا توجد بيانات متاحة لهذا المريض بعد.",
    createSuccess: "تم إنشاء الرسم البياني بنجاح",
    createError: "فشل إنشاء الرسم البياني",
    updateSuccess: "تم تحديث الرسم البياني بنجاح",
    updateError: "فشل تحديث الرسم البياني",
    deleteSuccess: "تم حذف الرسم البياني بنجاح",
    deleteError: "فشل حذف الرسم البياني",
    errName: "يجب أن يتكون الاسم من حرفين على الأقل",
    errEvaluation: "يرجى اختيار استبيان",
    errChartType: "يرجى اختيار نوع الرسم البياني",
    errElement: "يرجى اختيار سؤال",
    errOverlayQuestion: "يرجى اختيار استبيان وسؤال لمصدر البيانات هذا",
    overlay: {
      section: "بيانات إضافية",
      hint: "أضف بيانات من أسئلة أو وحدات أخرى لعرضها على نفس الرسم البياني.",
      add: "إضافة مصدر بيانات",
      remove: "إزالة",
      typeQuestion: "سؤال آخر",
      typeMedication: "الدواء الذي تم تناوله",
      typeActivity: "النشاط الذي تم تنفيذه",
      renderMarkers: "عرض كعلامات ملونة",
      renderLine: "عرض كخط",
      namePlaceholder: "تصفية حسب الاسم (اتركه فارغًا للكل)...",
      labelPlaceholder: "تسمية العرض (اختياري)...",
      colorLabel: "اللون",
    },
  },

  // Analytics
analytics: {
  title: "التحليلات",
  subtitle: "انقر على بطاقة لعرض الإحصائيات والرسوم البيانية التفصيلية.",
  loading: "جارٍ تحميل التحليلات...",
  error: "فشل تحميل التحليلات. يرجى المحاولة مرة أخرى.",

  cards: {
    activities: {
      title: "تحليلات الأنشطة",
      description: "تحليل استخدام النشاط والاتجاهات وبيانات مشاركة المرضى."
    },
    medications: {
      title: "تحليلات الأدوية",
      description: "تحليل استخدام الأدوية والالتزام بالعلاج والاتجاهات ذات الصلة."
    },
    evaluations: {
      title: "تحليلات القياسات",
      description: "مراجعة نتائج القياسات والتقديمات وردود فعل المرضى."
    },
    file_share: {
      title: "مشاركة الملفات",
      description: "الإحصائيات وتعقب نقل ومشاركة الملفات داخل النظام."
    },
    parkinson_sensors: {
      title: "تحليلات مستشعرات باركنسون",
      description: "تحليل قراءات المستشعرات واتجاهات الحركة لمراقبة باركنسون."
    }
  },
  activities: {
    title: "تحليلات الأنشطة",
    description: "تحليل استخدام النشاط والاتجاهات وبيانات مشاركة المرضى.",
    busiestDaysChart: "الأيام الأكثر ازدحاماً",
    usageChart: "الاستخدام بروֹر الوقت"
  },
  medications: {
    title: "تحليلات الأدوية",
    description: "تحليل استخدام الأدوية والالتزام بالعلاج والاتجاهات ذات الصلة.",
    busiestDaysChart: "الأيام الأكثر ازدحاماً",
    usageChart: "الاستخدام بروֹر الوقت"
  },
  evaluations: {
    title: "تحليلات القياسات",
    description: "مراجعة نتائج القياسات والتقديمات وردود فعل المرضى.",
    busiestDaysChart:" الأيام الأكثر ازدحاماً",
    usageChart: "الاستخدام بروֹר الوقت",
    summary: {
    total: "إجمالي القياسات",
    mostUsed: "القياسات الأكثر استخداماً",
    types: "أنواع القياسات",
    }
  },
  export: "تصدير",
  filters: {
  startDate: "تاريخ البداية",
  endDate: "تاريخ النهاية",
},
},

  // Evaluations
  evaluations: {
    title: "القياسات",
    createNew: "إنشاء جديد",
    searchPlaceholder: "البحث عن القياسات...",
    noData: "لا توجد قياسات بعد. قم بإنشاء أول واحد!",
    componentCount: "مكونات",
    screenCount: "شاشات",
    lastModified: "آخر تعديل",
    createdBy: "تم الإنشاء بواسطة",
    edit: "تعديل",
    delete: "حذف",
    duplicate: "تكرار",
    preview: "معاينة",
    deleteTitle: "حذف القياس",
    deleteDescription: "هل أنت متأكد أنك تريد حذف \"{{name}}\"؟ لا يمكن التراجع عن هذا الإجراء.",
    createTitle: "إنشاء قياس",
    nameLabel: "الاسم",
    namePlaceholder: "أدخل اسم القياس...",
    create: "إنشاء",
    cancel: "إلغاء",
    errName: "يجب أن يحتوي الاسم على حرفين على الأقل",
    createSuccess: "تم إنشاء القياس بنجاح",
    createError: "فشل إنشاء القياس. حاول مرة أخرى.",
    deleteSuccess: "تم حذف القياس بنجاح",
    deleteError: "فشل حذف القياس. حاول مرة أخرى.",
    updateSuccess: "تم تحديث القياس بنجاح",
    updateError: "فشل تحديث القياس. حاول مرة أخرى.",
    editTitle: "تعديل القياس",
    isPublic: "عام",
    addExisting: "إضافة موجود",
    addExistingTitle: "إضافة قياس موجود",
    noExistingData: "لا توجد قياسات متاحة.",
    searchExisting: "البحث عن قياسات موجودة...",
    add: "إضافة",
    typeLabel: "النوع",
    errType: "يرجى اختيار نوع القياس",
    public: "عام",
    active: "نشط",
    adoptSuccess: "تمت إضافة القياس بنجاح",
    adoptError: "فشل إضافة القياس. حاول مرة أخرى.",
    selectAll: "تحديد الكل",
    deselectAll: "إلغاء التحديد",
    addCount: "إضافة ({{count}})",
    adoptPartialError: "فشل إضافة بعض القياسات",
    showAdopted: "عرض القياسات المضافة مسبقاً",
    alreadyAdded: "تمت الإضافة مسبقاً",
    emptyColumn: "لا توجد قياسات في هذه الفئة",

    results_title: "نتائج التقييم",
    submitted_at: "تاريخ الإرسال",
    list_view: "عرض القائمة",
    analytics_view: "عرض التحليلات",
    score: "النتيجة",
    error_title: "خطأ: لم يتم العثور على الإرسال",
    error_desc: "يرجى التحقق من صحة المعرفات.",

    status: {
      excellent: "ممتاز",
      pass: "ناجح",
      fail: "راسب",
    },

    analytics: {
      avg_grade: "المعدل",
      passed: "الأسئلة التي تم اجتيازها",
      needs_improvement: "يحتاج إلى تحسين",
      questions: "الأسئلة",
      grade_distribution: "توزيع الدرجات",
      pass_fail_ratio: "نسبة النجاح/الرسوب",
      passed_label: "معدل النجاح",
      pass: "ناجح",
      fail: "راسب",
      count: "العدد",
    },

    table: {
      question_label: "السؤال",
      answer_provided: "الإجابة المقدمة",
      status: "الحالة",
      grade: "الدرجة",
      actions: "الإجراءات",
    },

    types: {
      questionnaries: "الاستبيانات",
      cognitiveTests: "اختبارات معرفية",
      moduleQuestionnaire: "استبيان الوحدة",
      medicalStaffEvaluation: "تقييم الطاقم الطبي",
    },

    builder: {
      title: "منشئ الشاشات",
      clearCanvas: "مسح اللوحة",
      previewMode: "معاينة",
      exitPreview: "الخروج من المعاينة",
      save: "حفظ",
      saveSuccess: "تم حفظ البنية بنجاح",
      saveError: "فشل حفظ البنية. حاول مرة أخرى.",
      back: "رجوع",
      emptyCanvas: "اسحب المكونات من اللوحة اليسرى للبدء",
      noSelection: "اختر مكوناً لتعديل خصائصه",
      properties: "الخصائص",
      screens: {
        addScreen: "إضافة شاشة",
        deleteScreen: "حذف شاشة",
        renameScreen: "إعادة تسمية الشاشة",
        screenTitle: "الشاشة {{number}}",
        deleteConfirm: "هل أنت متأكد أنك تريد حذف هذه الشاشة؟",
        cannotDeleteLast: "لا يمكن حذف آخر شاشة",
        screenIndicator: "الشاشة {{current}} من {{total}}",
      },
      preview: {
        next: "التالي",
        previous: "السابق",
        screenOf: "الشاشة {{current}} من {{total}}",
      },
      totalScore: "إجمالي الدرجة",
      versionControl: "التحكم بالإصدارات",
      branchVersion: "إنشاء إصدار جديد",
      deleteVersion: "حذف الإصدار",
      noVersions: "لا توجد إصدارات بعد",
      activeVersion: "نشط",
      deleteVersionConfirm: "حذف هذا الإصدار؟ سيعود المكون إلى إصدار آخر.",
      selectVersion: "اختر إصدار",
    },

    categories: {
      textDisplay: "نص وعرض",
      userInputs: "مدخلات المستخدم",
      actions: "إجراءات",
      structure: "بنية",
    },

    components: {
      heading: "عنوان",
      paragraph: "فقرة",
      infoCard: "معلومة",
      image: "صورة",
      icon: "أيقونة",
      textInput: "نص",
      numberInput: "رقم",
      dropdown: "قائمة منسدلة",
      multiSelect: "اختيار متعدد",
      radioGroup: "اختيار واحد",
      cardRadioGroup: "بطاقة اختيار",
      cardMultiSelect: "بطاقة متعددة",
      datePicker: "تاريخ",
      timePicker: "وقت",
      scale: "مقياس",
      toggleSwitch: "مفتاح تبديل",
      button: "زر",
      recordButton: "تسجيل",
      fileUpload: "رفع",
      keyboard: "لوحة مفاتيح",
      rowContainer: "صف",
      visualQuestion: "سؤال مرئي",
    },

    props: {
      label: "تسمية",
      text: "نص",
      placeholder: "نص إرشادي",
      required: "إلزامي",
      options: "خيارات",
      addOption: "إضافة خيار",
      min: "الحد الأدنى",
      max: "الحد الأقصى",
      step: "خطوة",
      level: "مستوى",
      variant: "نوع",
      src: "رابط المصدر",
      alt: "نص بديل",
      iconName: "اسم الأيقونة",
      size: "الحجم",
      layout: "التخطيط",
      defaultValue: "القيمة الافتراضية",
      correctAnswerType: "نوع الإجابة",
      correctAnswer: "الإجابة الصحيحة",
      grade: "الدرجة",
      optionScore: "نقاط الخيار",
      totalScore: "إجمالي الدرجة",
      minLabel: "تسمية الحد الأدنى",
      maxLabel: "تسمية الحد الأقصى",
      trueLabel: "تسمية صحيح",
      falseLabel: "تسمية خطأ",
      allowPartialScore: "السماح بنقاط جزئية",
      buttonLabel: "نص الزر",
      fileType: "نوع الملف",
      keyboardType: "نوع لوحة المفاتيح",
      visualKey: "المفتاح المرئي",
      customKey: "مفتاح مخصص",
      spots: "النقاط",
      addSpot: "إضافة نقطة",
      spotPoint: "مفتاح النقطة",
      subitems: "العناصر الفرعية",
      addSubitem: "إضافة عنصر فرعي",
    },

    device: {
      mobile: "هاتف",
      tablet: "جهاز لوحي",
      desktop: "كمبيوتر",
    },
    deleteConfirm: {
      title: "هل أنت متأكد أنك تريد حذف هذه الإجابة؟",
      description: "سيؤدي هذا إلى حذف الإجابة نهائيًا من التقييم.",
      cancel: "إلغاء",
      confirm: "حذف"
    },
    editScore: {
      title: "تعديل درجة السؤال",
      description: "قم بتحديث الدرجة لهذا الجواب.",
      placeholder: "أدخل الدرجة",
      cancel: "إلغاء",
      save: "حفظ"
    },
  },

  // Files
  fileShare: {
    title: "مشاركة الملفات",
    description:
      "شارك المستندات وأدرها مع المرضى. اختر مريضًا لعرض ملفاته ورفعها وتنزيلها.",
    viewPatients: "عرض المرضى",
  },
  parkinsonSensors: {
    title: "مستشعرات باركنسون",
    description:
      "جمع وتحليل بيانات مستشعرات الحركة القابلة للارتداء لتقييم الأعراض الحركية لمرض باركنسون. يقرأ تطبيق الهاتف باستمرار بيانات مقياس التسارع والجيروسكوب والخطوات لاكتشاف الرعشة والسقوط وشبه السقوط وحالات الدواء (تشغيل/إيقاف)، ثم يرفع النتائج للمراجعة السريرية. اختر مريضًا لعرض بيانات مستشعراته.",
    viewPatients: "عرض المرضى",
    calc: {
      heading: "كيف تُحسب القيم",
      intro:
        "تُقدَّر حالة الدواء (تشغيل/إيقاف) بمقارنة النشاط الحالي للمريض بخطه الأساسي الشخصي الذي يُبنى تدريجيًا على الجهاز.",
      metricsTitle: "١. حساب مقاييس النشاط",
      metricsBody:
        "خلال نافذة لا تقل عن ٦٠ عينة، يحسب متوسط مقدار التسارع، وتغيّر التسارع (الانحراف المعياري)، ومتوسط مقدار الجيروسكوب، وتردد الخطوات (تغيّر الخطوات في الدقيقة).",
      baselineTitle: "٢. حساب الخط الأساسي",
      baselineBody:
        "تُحسب متوسطات هذه المقاييس بمرور الوقت، مع انحرافاتها المعيارية، في خط أساسي شخصي يُخزَّن في قاعدة بيانات Room محلية. تكتمل المعايرة فقط بعد ٣٠ عينة على الأقل تمتد على ٤ ساعات على الأقل (دورة دواء واحدة).",
      classifyTitle: "٣. تصنيف حالة تشغيل/إيقاف",
      classifyBody:
        "يحوّل المُصنِّف المقاييس الحالية إلى درجة موزونة واحدة مقابل الخط الأساسي ويربطها بحالة تشغيل أو إيقاف أو غير محددة.",
      classifyPoints: [
        "درجة معيارية لكل بُعد: (القيمة الحالية − متوسط الخط الأساسي) / الانحراف المعياري للخط الأساسي. القيمة الموجبة تعني نشاطًا أعلى من المعتاد.",
        "درجة مركبة موزونة: مقدار التسارع ٠٫٣، تردد الخطوات ٠٫٣، تغيّر التسارع ٠٫٢، نشاط الجيروسكوب ٠٫٢.",
        "نطاق التباطؤ (من −٠٫٢ إلى +٠٫٢): لا تتغيّر الحالة إلا عند تجاوز الدرجة لهذا النطاق، وإلا تبقى على الحالة السابقة لتجنّب التذبذب. درجة > ٠٫٢ ← تشغيل، < −٠٫٢ ← إيقاف.",
        "الثقة = الأصغر من (|الدرجة| ÷ ٢، ١).",
        "تُرجِع \"غير محددة\" حتى تكتمل المعايرة.",
      ],
    },
    detectors: {
      heading: "الأحداث المكتشَفة",
      colEvent: "الحدث",
      colSensors: "المستشعرات الأساسية",
      colMethod: "الطريقة الأساسية",
      tremor: {
        name: "الرعشة",
        sensors: "التسارع الخطي + الجيروسكوب + الخطوات + الضوء",
        method:
          "نسبة قدرة تحويل فورييه السريع في نطاق ٤–٦٫٥ هرتز، مع مطابقة تردد الجيروسكوب، وبوابة مدة ٨ ثوانٍ.",
      },
      fall: {
        name: "السقوط",
        sensors: "التسارع الخام + التسارع الخطي + الجيروسكوب",
        method:
          "سقوط حر (< ٣ م/ث²) ← اصطدام (> ١٥ م/ث²) ← ٢٫٥ ثانية من السكون (متوسط منخفض وانحراف معياري منخفض).",
      },
      nearFall: {
        name: "شبه سقوط",
        sensors: "التسارع الخطي + الجيروسكوب",
        method:
          "٣ ذُرى على الأقل > ١٤ م/ث² مع جيروسكوب > ٣٫٥، يتبعها تعافٍ إلى الوضع الطبيعي خلال ٣ ثوانٍ.",
      },
      onOff: {
        name: "حالة تشغيل/إيقاف",
        sensors: "التسارع + الجيروسكوب + الخطوات",
        method:
          "درجات معيارية موزونة مقابل الخط الأساسي الشخصي لمدة ٤ ساعات، مع نطاق تباطؤ.",
      },
    },
    upload: {
      heading: "مسار الرفع",
      body:
        "تُحوَّل الأحداث المكتشَفة (مع مخازن مقياس التسارع/الجيروسكوب الخام وبيانات المستشعرات المجمَّعة) إلى طلب خادم موسوم بمعرّفات المريض والعيادة، ويُرفع عبر خدمة المستشعرات. ولوحدة حالة تشغيل/إيقاف نموذج DTO ومُحوِّل ومسار طلب خادم خاص بها.",
    },
    patientView: {
      empty: "لم يتم رفع أي بيانات مستشعرات بعد.",
      error_title: "تعذّر تحميل بيانات المستشعرات",
      error_desc: "حدث خطأ أثناء جلب بيانات المستشعرات. يرجى المحاولة مرة أخرى.",
      table: {
        event: "الحدث",
        detectedAt: "وقت الاكتشاف",
        sensors: "المستشعرات",
        details: "التفاصيل",
      },
    },
  },
  files: {
    title: "مستندات المريض",
    subtitle: "إدارة وعرض الملفات الطبية",

    upload_button: "تحميل مستند",
    dialog_title: "تحميل مستند جديد",
    dialog_desc: "اختر ملفًا لرفعه إلى سجل المريض",

    drag_drop: "اسحب الملف وأفلته هنا",
    supported_formats: "يدعم PDF و DOCX و CSV و PNG و JPG و SVG",
    browse: "تصفح الجهاز",

    choose_different: "اختر ملفًا آخر",
    save_as: "حفظ باسم",
    size: "{{size}} ميجابايت",

    search_placeholder: "ابحث عن ملفات...",

    missing_patient_context: "لم يتم اختيار مريض",
    select_patient_hint: "يرجى اختيار مريض لعرض ملفاته",

    empty: {
      title: "لا توجد مستندات"
    },

    table: {
      name: "الاسم",
      date: "التاريخ",
      owner: "المالك",
      actions: "إجراءات"
    }
  },

// frequencyManager
frequencyManager: {
  frequencyLabel: "التكرار والجدول الزمني",
  addTime: "إضافة وقت",
  dayOfMonth: "اليوم في الشهر",
  daysOfWeek: "أيام الأسبوع",
  frequencyOptions: {
    once: "مرة واحدة",
    daily: "يومي",
    weekly: "أسبوعي",
    monthly: "شهري",
  },
  days: {
    SUN: "الأحد",
    MON: "الاثنين",
    TUE: "الثلاثاء",
    WED: "الأربعاء",
    THU: "الخميس",
    FRI: "الجمعة",
    SAT: "السبت",
  },
}

} as const;

export default ar;