...
import type { QComponentType } from "@/common/types/questionnaire";
import {
...
export function useBuilderDnd() {
  ...
  const addComponent = useQuestionnaireBuilderStore((s) => s.addComponent);
  const addComponentToRow = useQuestionnaireBuilderStore(
    (s) => s.addComponentToRow,
  );
  const moveComponent = useQuestionnaireBuilderStore((s) => s.moveComponent);
