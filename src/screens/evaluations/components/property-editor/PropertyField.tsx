...
import type { IQOptionItem, IQVisualSpot, CorrectAnswerType, QComponentType } from "@/common/types/evaluation";
      ...
      {field.fieldType === "keySelector" && (
        <div className="space-y-1.5">
          ...
          >
            ...
            <option value="__custom__">{t("evaluations.props.customKey")}</option>
          </select>
          {isCustomKey && (
            <Input
              value={(value as string) ?? ""}
              onChange={handleCustomKeyChange}
              placeholder={t("evaluations.props.customKey")}
