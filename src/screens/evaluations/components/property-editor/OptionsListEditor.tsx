...
import type { IQOptionItem, QComponentType, CorrectAnswerType } from "@/common/types/evaluation";
...
export function OptionsListEditor({
  ...
  return (
    <div className="space-y-2">
        ...
        <div key={opt.id} className="flex items-center gap-1">
          ...
          {showScoring && opt.isCorrect && (
            <Input
              ...
              placeholder={t("evaluations.props.optionScore")}
            />
          )}
          <button
            type="button"
      ...
      >
        <Plus size={12} />
        {t("evaluations.props.addOption")}
      </Button>
      {showScoring && totalScore > 0 && (
        <div className="text-xs text-muted-foreground text-right">
          {t("evaluations.props.totalScore")}: {totalScore}
        </div>
      )}
    </div>
  );
