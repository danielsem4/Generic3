...
import type { QComponentType } from "@/common/types/evaluation";
import {
...
export function useBuilderDnd() {
  ...
  const addComponent = useEvaluationBuilderStore((s) => s.addComponent);
  const addComponentToRow = useEvaluationBuilderStore(
    (s) => s.addComponentToRow,
  );
  const moveComponent = useEvaluationBuilderStore((s) => s.moveComponent);
