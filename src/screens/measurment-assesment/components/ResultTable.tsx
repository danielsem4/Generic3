...
interface ChartDistributionEntry {
  range: string;
  count: number;
  fill: string;
}
    ...
    <div className="p-8 space-y-6 bg-card" dir="ltr">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
        <Card className="md:col-span-3 border-border shadow-sm bg-background/50">
          <CardHeader><CardTitle className="text-lg font-bold text-foreground">{t("measurements.analytics.grade_distribution")}</CardTitle></CardHeader>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={distributionData}>
                ...
                <Bar dataKey="count" radius={[6, 6, 0, 0]} barSize={50}>
                  {distributionData.map((entry: ChartDistributionEntry, i: number) => (
        ...
        <Card className="md:col-span-2 border-border shadow-sm bg-background/50">
          <CardHeader><CardTitle className="text-lg font-bold text-foreground">{t("measurements.analytics.pass_fail_ratio")}</CardTitle></CardHeader>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={pieData} innerRadius={75} outerRadius={95} paddingAngle={5} dataKey="value" startAngle={90} endAngle={-270}>
                    {pieData.map((entry: ChartPieEntry, i: number) => (
