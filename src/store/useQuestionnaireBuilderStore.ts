...
} from "@/screens/questionnaires/lib/treeUtils";

interface QuestionnaireBuilderStore {
  ...
  addQuestionnaire: (q: IQuestionnaire) => void;
  updateQuestionnaire: (id: string, updates: Partial<IQuestionnaire>) => void;
  deleteQuestionnaire: (id: string) => void;
  duplicateQuestionnaire: (id: string) => void;
...
export const useQuestionnaireBuilderStore = create<QuestionnaireBuilderStore>()(
  (set, get) => ({
    ...
    duplicateQuestionnaire: (id) => {
      const state = get();
      const original = state.questionnaires.find((q) => q.id === id);
      if (!original) return;

      const now = new Date().toISOString();
      const duplicate: IQuestionnaire = {
        ...original,
      ...
      set((s) => ({ questionnaires: [...s.questionnaires, duplicate] }));
    },

    loadQuestionnaire: (id) => {
      const q = get().questionnaires.find((q) => q.id === id);
      if (!q) return;
      set({
    ...
    saveCurrentQuestionnaire: () => {
      const state = get();
      if (!state.activeQuestionnaireId) return;
      const now = new Date().toISOString();
      set((s) => ({
        questionnaires: s.questionnaires.map((q) =>
          q.id === state.activeQuestionnaireId
