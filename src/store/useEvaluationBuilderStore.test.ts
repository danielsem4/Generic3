...
import { useEvaluationBuilderStore } from "./useEvaluationBuilderStore";
...
function seedScreens(screens: IQScreen[], activeScreenIndex = 0) {
  useEvaluationBuilderStore.setState({
    screens,
    activeScreenIndex,
...
describe("reorderScreens", () => {
  ...
  it("reorders screens so ids appear in new order", () => {
    ...
    useEvaluationBuilderStore.getState().reorderScreens(2, 0);

    const ids = useEvaluationBuilderStore.getState().screens.map((s) => s.id);
  ...
  it("keeps activeScreenIndex on the same screen when the active screen is moved", () => {
    ...
    const { screens, activeScreenIndex } = useEvaluationBuilderStore.getState();
  ...
  it("adjusts activeScreenIndex when a screen to its left moves past it (rightward)", () => {
    ...
    useEvaluationBuilderStore.getState().reorderScreens(0, 3);
  ...
  it("adjusts activeScreenIndex when a screen to its right moves past it (leftward)", () => {
    ...
    useEvaluationBuilderStore.getState().reorderScreens(3, 0);
  ...
  it("leaves activeScreenIndex unchanged when a move does not cross it", () => {
    ...
    useEvaluationBuilderStore.getState().reorderScreens(0, 1);
  ...
  it("sets isDirty to true after a successful reorder", () => {
    ...
    expect(useEvaluationBuilderStore.getState().isDirty).toBe(false);

    useEvaluationBuilderStore.getState().reorderScreens(0, 1);

    expect(useEvaluationBuilderStore.getState().isDirty).toBe(true);
  });
  ...
  it("preserves screen titles on reorder (no auto-rename of 'Screen N')", () => {
    ...
    const titles = useEvaluationBuilderStore
      .getState()
  ...
  it("no-ops when fromIndex === toIndex", () => {
    ...
    useEvaluationBuilderStore.getState().reorderScreens(1, 1);

    const state = useEvaluationBuilderStore.getState();
  ...
  it("no-ops when indices are out of bounds", () => {
    ...
    useEvaluationBuilderStore.getState().reorderScreens(0, 5);
